...
from pajin.control_plane.database_schema import (
    ...
    REVIEW_NOTIFICATION_RECEIPT_SCHEMA_VERSION,
    REVIEW_NOTIFICATION_RECEIPT_TABLES,
...
_MIGRATIONS = {
    ...
    REVIEW_NOTIFICATION_RECEIPT_SCHEMA_VERSION: "independent-review-notification-receipts",
}
...
def _initialize_schema(connection: Connection) -> None:  # noqa: C901
    ...
    if cp_tables == V15_CONTROL_PLANE_TABLES:
        ...
        _validate_current_schema(connection)
        return
    if cp_tables == V16_CONTROL_PLANE_TABLES:
        if _latest_schema_version(connection) != CHECKPOINT_KEY_IDENTITY_SCHEMA_VERSION:
            raise SchemaInitializationError("unknown migration history for schema-v16 table set")
        _validate_v16_schema(connection)
        _migrate_v16_schema(connection)
        _validate_current_schema(connection)
        return
...
def _migrate_v16_schema(connection: Connection) -> None:
    """Add empty notification receipts without rewriting any review revision."""
    _lock_v9_migration_writes(connection)
    _create_tables(connection, REVIEW_NOTIFICATION_RECEIPT_TABLES)
    for table_name in sorted(REVIEW_NOTIFICATION_RECEIPT_TABLES):
        _install_append_only_trigger(connection, table_name)
        _install_complete_append_only_guard(connection, table_name)
    _record_migration(connection, REVIEW_NOTIFICATION_RECEIPT_SCHEMA_VERSION)
...
def _validate_v16_schema(connection: Connection) -> None:
    _validate_tables(
        connection,
        V16_CONTROL_PLANE_TABLES,
        metadata=_V16_METADATA,
        append_only_guard_version=COMPLETE_APPEND_ONLY_GUARDS_SCHEMA_VERSION,
        require_submission_and_lease_guards=True,
    )
    rows = connection.execute(
    ...
    expected = [
        (version, _MIGRATIONS[version])
        for version in range(LEGACY_SCHEMA_VERSION, CHECKPOINT_KEY_IDENTITY_SCHEMA_VERSION + 1)
    ]
    actual = [(int(row.version), str(row.description)) for row in rows]
    if actual != expected or any(row.applied_at is None for row in rows):
        raise SchemaInitializationError("unknown or incomplete schema v16 migration history")
    _validate_v10_authority_rows(connection)
