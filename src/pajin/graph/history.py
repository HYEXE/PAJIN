...
def read_snapshot_history(
    ...
    *,
    ...
    with _readonly_connection(database) as connection:
        ...
        result = _read_snapshot_history_from_connection(
            connection,
...
def _history_state(
    campaign_id: str, head: str | None, event_head: str | None, projection: str
) -> str:
    return graph_digest(
...
def _read_snapshot_history_from_connection(
    ...
    *,
    ...
    """Complete historical verification inside an already validated read transaction."""
    entries = []
    ...
    if current.revision != len(events) or current.event_log_head_digest != event_head:
        raise SQLiteGraphStoreError("Graph projection recovery is required before history reads")
    state = _history_state(campaign_id, head, event_head, current.projection_digest)
