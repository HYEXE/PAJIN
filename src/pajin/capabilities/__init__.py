"""Versioned executable Capability contracts and compatibility adapters."""

from pajin.capabilities.activation import (
    CAPABILITY_DISPATCH_AUDIT_EVENT_API_VERSION,
    EXISTING_MODE_CAPABILITY_ACTIVATION_SET_API_VERSION,
    EXISTING_MODE_CAPABILITY_MCP_ACTIVATION_SET_API_VERSION,
    PREPARED_CAPABILITY_ACTION_API_VERSION,
    CapabilityDispatchAuditEvent,
    CapabilityDispatchAuditStore,
    CapabilityDispatchStage,
    ExistingModeCapabilityActivation,
    ExistingModeCapabilityActivationBinding,
    ExistingModeCapabilityActivationError,
    ExistingModeCapabilityActivationSet,
    ExistingModeCapabilityGatewayDispatcher,
    PreparedCapabilityAction,
    activate_existing_mode_capabilities,
    capability_gateway_outcome_digest,
    capability_grant_digest,
    capability_normalized_parameters_digest,
    capability_tool_request_digest,
)
from pajin.capabilities.adapters import (
    ToolCapabilityRegistration,
    capability_definition_from_tool,
    capability_registry_from_tools,
    registered_action_capability,
    registered_action_capability_registry,
    tool_spec_digest,
)
from pajin.capabilities.authorities import (
    CODE_BACKED_CAPABILITY_API_VERSION,
    CapabilityActionCompiler,
    CapabilityAuthorityAdapter,
    CapabilityAuthorityBinding,
    CapabilityAuthorityError,
    CapabilityAuthorityRegistry,
    CapabilityAuthorityRole,
    CapabilityCleanupHandler,
    CapabilityExecutorAdapter,
    CapabilityMaterializer,
    CapabilityOracleDecision,
    CapabilityReplayStrategy,
    CapabilityResultNormalizer,
    CapabilitySuccessOracle,
    CodeBackedCapability,
    CodeBackedCapabilityRef,
    RegisteredCapabilityAuthority,
    capability_authority_binding,
)
from pajin.capabilities.cleanup_dispatch import (
    CLEANUP_CAPABILITY_DISPATCH_AUDIT_EVENT_API_VERSION,
    CLEANUP_CAPABILITY_DISPATCH_EVENT_PREFIX,
    CLEANUP_CAPABILITY_DISPATCH_RECONCILIATION_API_VERSION,
    CLEANUP_CAPABILITY_DISPATCH_RECONCILIATION_EVENT_TYPE,
    CleanupCapabilityDispatchAuditEvent,
    CleanupCapabilityDispatchError,
    CleanupCapabilityDispatchReconciliation,
    CleanupCapabilityDispatchReconciliationObservation,
    CleanupCapabilityDispatchReconciliationStatus,
    CleanupCapabilityDispatchStage,
    ExistingModeCleanupCapabilityGatewayDispatcher,
    reconcile_cleanup_capability_dispatch,
)
from pajin.capabilities.domain_projection import (
    CAPABILITY_DOMAIN_CLASSIFICATION_API_VERSION,
    CAPABILITY_DOMAIN_INVENTORY_PROJECTION_API_VERSION,
    CapabilityDomainClassificationRef,
    CapabilityDomainInventoryProjection,
    CapabilityDomainProjectionError,
    RegisteredCapabilityDomainClassification,
    registered_capability_domain_inventory_projection,
    resolve_registered_capability_domain_classification,
)
from pajin.capabilities.existing import (
    EXISTING_KISA_REPLAY_PLAN_API_VERSION,
    EXISTING_MODE_CAPABILITY_ADAPTER_VERSION,
    REGISTERED_MCP_CAPABILITY_ID,
    REGISTERED_MCP_CAPABILITY_VERSION,
    REGISTERED_MCP_TARGET,
    ExistingCapabilitySuccessPolicy,
    ExistingModeCapabilityBundle,
    existing_mode_capability_bundle,
    existing_mode_capability_registrations,
)
from pajin.capabilities.exit_gate import (
    CAPABILITY_OPERATIONAL_EVIDENCE_ARTIFACT,
    CAPABILITY_OPERATIONAL_EVIDENCE_SET_API_VERSION,
    WEB_AI_HYBRID_CAMPAIGN_EXIT_GATE_API_VERSION,
    WEB_AI_HYBRID_CAPABILITY_IDS,
    CapabilityOperationalEvidenceSet,
    WebAIHybridCampaignExitGate,
    WebAIHybridCampaignExitGateError,
    WebAIHybridDispatchResult,
    verify_web_ai_hybrid_campaign_exit_gate,
)
from pajin.capabilities.lifecycle import (
    CAPABILITY_LIFECYCLE_POLICY_API_VERSION,
    CAPABILITY_RELEASE_API_VERSION,
    CAPABILITY_REVIEW_API_VERSION,
    CAPABILITY_TRUST_KEY_API_VERSION,
    CapabilityDeprecationNotice,
    CapabilityLifecycleError,
    CapabilityLifecycleKeyRole,
    CapabilityLifecycleKeyState,
    CapabilityLifecyclePolicy,
    CapabilityLifecycleRegistry,
    CapabilityLifecycleSigner,
    CapabilityLifecycleTrustKey,
    CapabilityMaturityApproval,
    CapabilityReleaseBundle,
    CapabilityReleaseRef,
    CapabilityReleaseStatement,
    CapabilityReviewDecision,
    CapabilityReviewStatement,
    CapabilityUseProfile,
    ResolvedCapabilityRelease,
    SignedCapabilityRelease,
    SignedCapabilityReview,
    capability_lifecycle_public_key,
)
from pajin.capabilities.metrics import (
    CAPABILITY_DELIVERY_EVIDENCE_API_VERSION,
    CAPABILITY_METRIC_SCOPE_API_VERSION,
    CAPABILITY_METRICS_REPORT_API_VERSION,
    CAPABILITY_ORACLE_OBSERVATION_API_VERSION,
    CAPABILITY_REPLAY_OBSERVATION_API_VERSION,
    CAPABILITY_REPLAY_SUPPORT_API_VERSION,
    CapabilityDeliveryEvidence,
    CapabilityDurationSummary,
    CapabilityLeadTimeMetrics,
    CapabilityLifecycleMetrics,
    CapabilityMetricDimension,
    CapabilityMetricGap,
    CapabilityMetricRequirement,
    CapabilityMetricScope,
    CapabilityMetricsInputBinding,
    CapabilityMetricsReportStatus,
    CapabilityOracleMetrics,
    CapabilityOracleObservation,
    CapabilityRatioMetric,
    CapabilityRegistryCoverageMetrics,
    CapabilityRegistryMetricsReport,
    CapabilityReplayMetrics,
    CapabilityReplayObservation,
    CapabilityReplaySupport,
    CapabilityReplayVerdict,
    build_capability_registry_metrics,
    existing_mode_capability_metric_scope,
    existing_mode_capability_metrics_baseline,
    existing_mode_capability_replay_support,
)
from pajin.capabilities.models import (
    CAPABILITY_DEFINITION_API_VERSION,
    CapabilityDefinition,
    CapabilityDefinitionError,
    CapabilityDefinitionRef,
    CapabilityDefinitionRegistry,
    CapabilityMaturity,
    CapabilitySideEffectClass,
    CapabilityToolBinding,
    canonical_capability_json,
    capability_definition_digest,
)
from pajin.capabilities.reconciliation import (
    CAPABILITY_DISPATCH_RECONCILIATION_API_VERSION,
    CAPABILITY_DISPATCH_RECONCILIATION_EVENT_TYPE,
    CAPABILITY_GRAPH_RUN_AUDIT_ANCHOR_API_VERSION,
    CAPABILITY_GRAPH_RUN_AUDIT_ANCHOR_EVENT_TYPE,
    CapabilityDispatchReconciliation,
    CapabilityDispatchReconciliationError,
    CapabilityDispatchReconciliationObservation,
    CapabilityDispatchReconciliationStatus,
    CapabilityGraphRunAuditAnchor,
    reconcile_capability_dispatch,
)
from pajin.capabilities.rollout import (
    EXISTING_MODE_CAPABILITY_MCP_RELEASE_SET_API_VERSION,
    EXISTING_MODE_CAPABILITY_RELEASE_SET_API_VERSION,
    ExistingModeCapabilityReleaseBinding,
    ExistingModeCapabilityReleaseSet,
    ExistingModeCapabilityRollout,
    ExistingModeCapabilityRolloutError,
    admit_existing_mode_capability_releases,
    existing_mode_capability_benchmark_mappings,
    existing_mode_capability_rollout_metrics,
)
from pajin.capabilities.scaffold import (
    CAPABILITY_AUTHORITY_TEMPLATE_API_VERSION,
    CAPABILITY_BENCHMARK_MAPPING_API_VERSION,
    CAPABILITY_SCAFFOLD_API_VERSION,
    CAPABILITY_SCAFFOLD_MANIFEST_API_VERSION,
    ActionCompilerTemplate,
    CapabilityAuthorityTemplate,
    CapabilityBenchmarkMapping,
    CapabilityScaffold,
    CapabilityScaffoldError,
    CapabilityScaffoldFile,
    CapabilityScaffoldSpec,
    CleanupHandlerTemplate,
    ExecutorAdapterTemplate,
    MaterializerTemplate,
    ReplayStrategyTemplate,
    ResultNormalizerTemplate,
    SuccessOracleTemplate,
    canonical_capability_parameter_schema,
    capability_parameter_schema_digest,
    generate_capability_scaffold,
    load_capability_scaffold_spec,
    write_capability_scaffold,
)

__all__ = [
    "CAPABILITY_AUTHORITY_TEMPLATE_API_VERSION",
    "CAPABILITY_BENCHMARK_MAPPING_API_VERSION",
    "CAPABILITY_DEFINITION_API_VERSION",
    "CAPABILITY_DELIVERY_EVIDENCE_API_VERSION",
    "CAPABILITY_DISPATCH_AUDIT_EVENT_API_VERSION",
    "CAPABILITY_DISPATCH_RECONCILIATION_API_VERSION",
    "CAPABILITY_DISPATCH_RECONCILIATION_EVENT_TYPE",
    "CAPABILITY_DOMAIN_CLASSIFICATION_API_VERSION",
    "CAPABILITY_DOMAIN_INVENTORY_PROJECTION_API_VERSION",
    "CAPABILITY_GRAPH_RUN_AUDIT_ANCHOR_API_VERSION",
    "CAPABILITY_GRAPH_RUN_AUDIT_ANCHOR_EVENT_TYPE",
    "CAPABILITY_LIFECYCLE_POLICY_API_VERSION",
    "CAPABILITY_METRICS_REPORT_API_VERSION",
    "CAPABILITY_METRIC_SCOPE_API_VERSION",
    "CAPABILITY_OPERATIONAL_EVIDENCE_ARTIFACT",
    "CAPABILITY_OPERATIONAL_EVIDENCE_SET_API_VERSION",
    "CAPABILITY_ORACLE_OBSERVATION_API_VERSION",
    "CAPABILITY_RELEASE_API_VERSION",
    "CAPABILITY_REPLAY_OBSERVATION_API_VERSION",
    "CAPABILITY_REPLAY_SUPPORT_API_VERSION",
    "CAPABILITY_REVIEW_API_VERSION",
    "CAPABILITY_SCAFFOLD_API_VERSION",
    "CAPABILITY_SCAFFOLD_MANIFEST_API_VERSION",
    "CAPABILITY_TRUST_KEY_API_VERSION",
    "CLEANUP_CAPABILITY_DISPATCH_AUDIT_EVENT_API_VERSION",
    "CLEANUP_CAPABILITY_DISPATCH_EVENT_PREFIX",
    "CLEANUP_CAPABILITY_DISPATCH_RECONCILIATION_API_VERSION",
    "CLEANUP_CAPABILITY_DISPATCH_RECONCILIATION_EVENT_TYPE",
    "CODE_BACKED_CAPABILITY_API_VERSION",
    "EXISTING_KISA_REPLAY_PLAN_API_VERSION",
    "EXISTING_MODE_CAPABILITY_ACTIVATION_SET_API_VERSION",
    "EXISTING_MODE_CAPABILITY_ADAPTER_VERSION",
    "EXISTING_MODE_CAPABILITY_MCP_ACTIVATION_SET_API_VERSION",
    "EXISTING_MODE_CAPABILITY_MCP_RELEASE_SET_API_VERSION",
    "EXISTING_MODE_CAPABILITY_RELEASE_SET_API_VERSION",
    "PREPARED_CAPABILITY_ACTION_API_VERSION",
    "REGISTERED_MCP_CAPABILITY_ID",
    "REGISTERED_MCP_CAPABILITY_VERSION",
    "REGISTERED_MCP_TARGET",
    "WEB_AI_HYBRID_CAMPAIGN_EXIT_GATE_API_VERSION",
    "WEB_AI_HYBRID_CAPABILITY_IDS",
    "ActionCompilerTemplate",
    "CapabilityActionCompiler",
    "CapabilityAuthorityAdapter",
    "CapabilityAuthorityBinding",
    "CapabilityAuthorityError",
    "CapabilityAuthorityRegistry",
    "CapabilityAuthorityRole",
    "CapabilityAuthorityTemplate",
    "CapabilityBenchmarkMapping",
    "CapabilityCleanupHandler",
    "CapabilityDefinition",
    "CapabilityDefinitionError",
    "CapabilityDefinitionRef",
    "CapabilityDefinitionRegistry",
    "CapabilityDeliveryEvidence",
    "CapabilityDeprecationNotice",
    "CapabilityDispatchAuditEvent",
    "CapabilityDispatchAuditStore",
    "CapabilityDispatchReconciliation",
    "CapabilityDispatchReconciliationError",
    "CapabilityDispatchReconciliationObservation",
    "CapabilityDispatchReconciliationStatus",
    "CapabilityDispatchStage",
    "CapabilityDomainClassificationRef",
    "CapabilityDomainInventoryProjection",
    "CapabilityDomainProjectionError",
    "CapabilityDurationSummary",
    "CapabilityExecutorAdapter",
    "CapabilityGraphRunAuditAnchor",
    "CapabilityLeadTimeMetrics",
    "CapabilityLifecycleError",
    "CapabilityLifecycleKeyRole",
    "CapabilityLifecycleKeyState",
    "CapabilityLifecycleMetrics",
    "CapabilityLifecyclePolicy",
    "CapabilityLifecycleRegistry",
    "CapabilityLifecycleSigner",
    "CapabilityLifecycleTrustKey",
    "CapabilityMaterializer",
    "CapabilityMaturity",
    "CapabilityMaturityApproval",
    "CapabilityMetricDimension",
    "CapabilityMetricGap",
    "CapabilityMetricRequirement",
    "CapabilityMetricScope",
    "CapabilityMetricsInputBinding",
    "CapabilityMetricsReportStatus",
    "CapabilityOperationalEvidenceSet",
    "CapabilityOracleDecision",
    "CapabilityOracleMetrics",
    "CapabilityOracleObservation",
    "CapabilityRatioMetric",
    "CapabilityRegistryCoverageMetrics",
    "CapabilityRegistryMetricsReport",
    "CapabilityReleaseBundle",
    "CapabilityReleaseRef",
    "CapabilityReleaseStatement",
    "CapabilityReplayMetrics",
    "CapabilityReplayObservation",
    "CapabilityReplayStrategy",
    "CapabilityReplaySupport",
    "CapabilityReplayVerdict",
    "CapabilityResultNormalizer",
    "CapabilityReviewDecision",
    "CapabilityReviewStatement",
    "CapabilityScaffold",
    "CapabilityScaffoldError",
    "CapabilityScaffoldFile",
    "CapabilityScaffoldSpec",
    "CapabilitySideEffectClass",
    "CapabilitySuccessOracle",
    "CapabilityToolBinding",
    "CapabilityUseProfile",
    "CleanupCapabilityDispatchAuditEvent",
    "CleanupCapabilityDispatchError",
    "CleanupCapabilityDispatchReconciliation",
    "CleanupCapabilityDispatchReconciliationObservation",
    "CleanupCapabilityDispatchReconciliationStatus",
    "CleanupCapabilityDispatchStage",
    "CleanupHandlerTemplate",
    "CodeBackedCapability",
    "CodeBackedCapabilityRef",
    "ExecutorAdapterTemplate",
    "ExistingCapabilitySuccessPolicy",
    "ExistingModeCapabilityActivation",
    "ExistingModeCapabilityActivationBinding",
    "ExistingModeCapabilityActivationError",
    "ExistingModeCapabilityActivationSet",
    "ExistingModeCapabilityBundle",
    "ExistingModeCapabilityGatewayDispatcher",
    "ExistingModeCapabilityReleaseBinding",
    "ExistingModeCapabilityReleaseSet",
    "ExistingModeCapabilityRollout",
    "ExistingModeCapabilityRolloutError",
    "ExistingModeCleanupCapabilityGatewayDispatcher",
    "MaterializerTemplate",
    "PreparedCapabilityAction",
    "RegisteredCapabilityAuthority",
    "RegisteredCapabilityDomainClassification",
    "ReplayStrategyTemplate",
    "ResolvedCapabilityRelease",
    "ResultNormalizerTemplate",
    "SignedCapabilityRelease",
    "SignedCapabilityReview",
    "SuccessOracleTemplate",
    "ToolCapabilityRegistration",
    "WebAIHybridCampaignExitGate",
    "WebAIHybridCampaignExitGateError",
    "WebAIHybridDispatchResult",
    "activate_existing_mode_capabilities",
    "admit_existing_mode_capability_releases",
    "build_capability_registry_metrics",
    "canonical_capability_json",
    "canonical_capability_parameter_schema",
    "capability_authority_binding",
    "capability_definition_digest",
    "capability_definition_from_tool",
    "capability_gateway_outcome_digest",
    "capability_grant_digest",
    "capability_lifecycle_public_key",
    "capability_normalized_parameters_digest",
    "capability_parameter_schema_digest",
    "capability_registry_from_tools",
    "capability_tool_request_digest",
    "existing_mode_capability_benchmark_mappings",
    "existing_mode_capability_bundle",
    "existing_mode_capability_metric_scope",
    "existing_mode_capability_metrics_baseline",
    "existing_mode_capability_registrations",
    "existing_mode_capability_replay_support",
    "existing_mode_capability_rollout_metrics",
    "generate_capability_scaffold",
    "load_capability_scaffold_spec",
    "reconcile_capability_dispatch",
    "reconcile_cleanup_capability_dispatch",
    "registered_action_capability",
    "registered_action_capability_registry",
    "registered_capability_domain_inventory_projection",
    "resolve_registered_capability_domain_classification",
    "tool_spec_digest",
    "verify_web_ai_hybrid_campaign_exit_gate",
    "write_capability_scaffold",
]
