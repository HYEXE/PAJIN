...
from pajin.benchmark.effectiveness_derived.plan import comparison_pins as prior_pins
from pajin.tools.disclosure_transforms import detect_transform_disclosure_suspicion


class ComparisonSuite(FrozenModel):
    version: Literal["effect-006-corpus-v1"] = "effect-006-corpus-v1"
    cases: tuple[Case, ...] = Field(min_length=18, max_length=18)
    prior_prompt_digests: tuple[Digest, ...] = Field(min_length=72, max_length=72)
    strata: tuple[
    ...
    def require_disjoint_private_corpus(self) -> Self:
        ...
        if self.strata != expected_strata or len(set(self.prior_prompt_digests)) != 72:
        ...
        if any(digest(prompt) in self.prior_prompt_digests for prompt in prompts):
            raise ValueError("new corpus overlaps EFFECT-002/003/004/005 development material")
...
def comparison_pins() -> tuple[SourcePin, ...]:
    detector = inspect.getsourcefile(detect_transform_disclosure_suspicion)
    if detector is None:
        raise ValueError("detector source unavailable")
    return (
        *prior_pins(),
        SourcePin(
            name="pajin.tools.disclosure_transforms",
...
class ComparisonPlan(FrozenModel):
    ...
    baseline: Literal["public-derived-opaque-output-v4"] = "public-derived-opaque-output-v4"
    candidate: Literal["public-transform-opaque-output-v5"] = "public-transform-opaque-output-v5"
