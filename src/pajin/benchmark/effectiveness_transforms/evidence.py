...
from pajin.benchmark.effectiveness_transforms.plan import ComparisonPlan, comparison_pins
from pajin.benchmark.effectiveness_transforms.scoring import (
    DetectorInput,
    PairedDetection,
    verify_pair,
)
...
class ComparisonIndex(FrozenModel):
    version: Literal["effect-006-v1"] = "effect-006-v1"
    source: EvaluationIndex
    detections: tuple[PairedDetection | None, ...] = Field(max_length=384)


def _quality_improved(before: dict[str, object], after: dict[str, object]) -> bool:
    values = [
    ...
    return (
        a_f1 > b_f1
        and a_precision > b_precision
        and a_recall >= b_recall
        and cast(int, after["fp"]) < cast(int, before["fp"])
    )
...
def recompute_comparison(root: Path, reference: RunReference) -> dict[str, object]:
    ...
    timings = {
        name: distribution([getattr(pair, name) for pair in index.detections if pair is not None])
        for name in (
    ...
    old_cpu, new_cpu = (
        timings[name]["mean"] for name in ("baseline_cpu_seconds", "candidate_cpu_seconds")
    )
    faster = isinstance(old_cpu, float) and isinstance(new_cpu, float) and new_cpu < old_cpu
    quality = bool(complete) and _quality_improved(before, after)
    return {
        "version": "effect-006-v1",
        "complete": bool(complete),
        "comparison_eligible": bool(complete),
        "quality_improved": quality,
        "cpu_improved": bool(complete) and faster,
        "improvement_confirmed": quality and faster,
        ...
        "failure_diagnostics": dict(
            Counter(
                audit_safe_worker_failure(
                    trial.gateway.worker_result.stderr
                    if trial.gateway and trial.gateway.worker_result
                    else "",
                    exit_code=(
                        trial.gateway.worker_result.exit_code
                        if trial.gateway and trial.gateway.worker_result
                        else None
                    ),
                    truncated=(
                        trial.gateway.worker_result.stderr_truncated
                        if trial.gateway and trial.gateway.worker_result
                        else False
                    ),
                )
        ...
        "detector_timing": timings,
        "detector_timing_scope": (
            "one untimed call then 64 calls per detector; per-call mean; "
            "detector order alternates by response"
        ),
