...
from pajin.benchmark.effectiveness_structured.evidence import ComparisonIndex
...
from pajin.benchmark.effectiveness_transforms.plan import ComparisonPlan as PriorPlan
...
def freeze_comparison(
    ...
    *,
    ...
    # Sealed prior plan/corpus commitments exclude consumed tasks even when that
    # experiment failed completeness. No prior quality result is reused or relabeled.
    expected = (
        *previous_plan.suite.prior_prompt_digests,
        *(digest(normalized_prompt(case.prompt)) for case in previous_plan.suite.cases),
    )
...
def consume_corpus(root: Path, plan: ComparisonPlan) -> None:
    ...
    marker = root / ".effect-007-consumed.sqlite3"
