...
from pajin.benchmark.effectiveness_transforms.plan import comparison_pins as prior_pins
from pajin.tools.disclosure_structured import detect_structured_disclosure_suspicion


class ComparisonSuite(FrozenModel):
    version: Literal["effect-007-corpus-v1"] = "effect-007-corpus-v1"
    cases: tuple[Case, ...] = Field(min_length=18, max_length=18)
    prior_prompt_digests: tuple[Digest, ...] = Field(min_length=90, max_length=90)
    strata: tuple[
    ...
    def require_disjoint_private_corpus(self) -> Self:
        ...
        if self.strata != expected_strata or len(set(self.prior_prompt_digests)) != 90:
        ...
        if any(digest(prompt) in self.prior_prompt_digests for prompt in prompts):
            raise ValueError("new corpus overlaps EFFECT-002/003/004/005/006 development material")
...
def comparison_pins() -> tuple[SourcePin, ...]:
    detector = inspect.getsourcefile(detect_structured_disclosure_suspicion)
    if detector is None:
        raise ValueError("detector source unavailable")
    return (
        *prior_pins(),
        SourcePin(
            name="pajin.tools.disclosure_structured",
...
class ComparisonPlan(FrozenModel):
    ...
    baseline: Literal["public-transform-opaque-output-v5"] = "public-transform-opaque-output-v5"
    candidate: Literal["public-structured-opaque-output-v6"] = "public-structured-opaque-output-v6"
