...
from pajin.benchmark.effectiveness_revision.plan import comparison_pins as prior_pins
from pajin.tools.disclosure_precision import detect_precision_disclosure_suspicion


class ComparisonSuite(FrozenModel):
    version: Literal["effect-004-corpus-v1"] = "effect-004-corpus-v1"
    cases: tuple[Case, ...] = Field(min_length=18, max_length=18)
    prior_prompt_digests: tuple[Digest, ...] = Field(min_length=36, max_length=36)
    strata: tuple[
    ...
    def require_disjoint_private_corpus(self) -> Self:
        ...
        if self.strata != expected_strata or len(set(self.prior_prompt_digests)) != 36:
        ...
        if any(digest(prompt) in self.prior_prompt_digests for prompt in prompts):
            raise ValueError("new corpus overlaps EFFECT-002/003 development material")
...
def comparison_pins() -> tuple[SourcePin, ...]:
    detector = inspect.getsourcefile(detect_precision_disclosure_suspicion)
    if detector is None:
        raise ValueError("detector source unavailable")
    sources = [("pajin.tools.disclosure_precision", Path(detector))]
    # Freeze the complete package used by the governed runtime, including diagnostics.
    package = Path(pajin.__file__).parent
    sources.extend(
        ("package/" + p.relative_to(package).as_posix(), p) for p in sorted(package.rglob("*.py"))
    )
    return (
        *prior_pins(),
        *(
...
class ComparisonPlan(FrozenModel):
    version: Literal["effect-004-v1"] = "effect-004-v1"
    ...
    candidate: Literal["mixed-group-opaque-output-v3"] = "mixed-group-opaque-output-v3"
