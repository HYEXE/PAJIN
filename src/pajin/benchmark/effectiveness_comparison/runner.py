...
from pajin.benchmark.effectiveness_comparison.evidence import ComparisonIndex
...
def freeze_comparison(
    ...
    *,
    ...
    if recompute_report(development_root, development_reference)["complete"] is not True:
...
def consume_corpus(root: Path, plan: ComparisonPlan) -> None:
    corpus = digest([normalized_prompt(case.prompt) for case in plan.suite.cases[2:]])
    # A changed nonce, model setting or plan does not make an observed corpus unused.
    marker = root / f".effect-002-started-{corpus}"
    with marker.open("x", encoding="ascii") as stream:
        os.chmod(marker, 0o600)
        stream.write(plan.commitment + "\n")
