"""Complete CAP-002 roles for the additive, approval-required SYS-002 Tool."""
...
class SystemAuthority:
    ...
    def stable_execution_context(self) -> Mapping[str, object]:
        return {
            "version": "pajin.sys-002-authority/v1",
            "role": self.authority_role.value,
            "tool": self.tool.stable_execution_context(),
            "oracle": "authenticated-os-release-metadata",
    ...
    def materialize(self, parameters: Mapping[str, JsonValue]) -> Mapping[str, JsonValue]:
        return cast(
            ...
            SystemInput.model_validate(dict(parameters)).model_dump(mode="json", by_alias=True),
        )

    def compile(
        self, request: ToolRequest, materialized_arguments: Mapping[str, JsonValue]
    ) -> ToolRequest:
        value = SystemInput.model_validate(dict(materialized_arguments))
    ...
    def evaluate(self, request: ToolRequest, result: ToolResult) -> CapabilityOracleDecision:
        ...
        output = SystemWorkerOutput.model_validate(result.data["receipt"])
        if (
            ...
            or result.data["metadata"] != distribution_metadata(output.agent.content())
        ):
    ...
    def plan_replay(
        ...
        if not result.success:
            return None
        return {
            "version": "pajin.sys-002-reexecution-plan/v1",
            "toolId": TOOL_ID,
...
def system_capability_bundle(tool: SystemReadTool) -> SystemBundle:
    if type(tool) is not SystemReadTool:
        raise TypeError("SYS-002 Capability requires its exact Tool implementation")
    schema = SystemInput.model_json_schema(by_alias=True)
    ...
    definition = capability_definition_from_tool(
        ...
        ToolCapabilityRegistration(
            ...
            threatClasses=("distribution-metadata",),
    ...
    return SystemBundle(
        ...
        CapabilityAuthorityRegistry(
            definitions,
            tuple(SystemAuthority(role, definition, tool) for role in CapabilityAuthorityRole),
        ),
    )
