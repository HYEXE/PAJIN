...
from pajin.system_aslr import models
...
def aslr_worker_limits() -> WorkerLimits:
    return WorkerLimits(
        timeout_seconds=25,
        memory_mb=128,
        cpus=0.5,
...
class AslrReadTool(Tool):
    spec = ToolSpec(
        tool_id=TOOL_ID,
        version="1.0.0",
        description="Read one authenticated Linux agent's fixed kernel ASLR setting",
    ...
    def __init__(self, deployment: AslrDeployment) -> None:
        self.deployment = AslrDeployment.model_validate_json(deployment.model_dump_json())
    ...
    def stable_execution_context(self) -> dict[str, object]:
        ...
        return {
            "spec": spec,
            "version": "pajin.sys-004-tool/v1",
            "deployment": self.deployment.model_dump(mode="json"),
            "limits": aslr_worker_limits().model_dump(mode="json"),
            "transport": "fixed-mtls-get-over-observed-connect",
            "hostParserSha256": sha256(Path(models.__file__).read_bytes()).hexdigest(),
            "findingAuthority": False,
        }

    def validate_request(self, request: ToolRequest) -> AslrInput:
        value = AslrInput.model_validate(request.arguments)
        if (
    ...
    def prepare(self, request: ToolRequest) -> WorkerJob:
        ...
        return WorkerJob(
            ...
            limits=aslr_worker_limits(),
            secret_requests=[
                WorkerSecretRequest(secret_ref=SECRET_REF, binding="system-mtls", ttl_seconds=30)
            ],
        )

    def checked_output(self, request: ToolRequest, result: WorkerResult) -> AslrWorkerOutput:
        self.validate_request(request)
        output = AslrWorkerOutput.model_validate(
        ...
        aslr_metadata(output.agent.content())
        return output
