...
> 수준의 Confirmed에는 별도 Restricted Reproducer의 새 요청·증적과 Oracle 성공도 필요하다.

> 과거 범위 참고: 아래의 최초 A01/A02 수직 조각과 재검증을 후속 작업으로 언급한 내용은 이
> ADR이 승인될 당시의 상태를 설명한다. 현재 구현은 A04, M03, M06도 다루며 재검증 폐루프도
> 포함한다. 자세한 내용은 [KISA 가이드 추적성](../KISA_TRACEABILITY.ko.md)을 참고한다.
...
2. Specialist는 Finding을 확정할 수 없다. 별도 Semantic Validator가 같은 Run의 증적만
   사용해 심사하고 PAJIN의 결정론적 Gate가 증적 출처와 Scope를 다시 확인한다. 이는 독립
   재현이 아니며 ADR 0027의 Restricted Reproducer가 별도로 필요하다.
3. 반복 실행에서 나온 동일 Finding은 제목·위협·대상을 기준으로 합치되 모든 재현 증적을
   보존하고 보수적인 신뢰도를 적용한다.

### 평가와 체크리스트

1. 공격 성공률, 차단·거부율, 재현율, 민감 정보 노출 수, 평균 지연, 위협 커버리지를
   구조화된 지표로 계산한다.
...
- 가이드 요구 사항이 Campaign 선택부터 실행·검증·보고까지 추적된다.
- 수록된 위협과 실제 테스트 커버리지가 분리되어 누락이 숨겨지지 않는다.
- 반복 실행과 분리된 증거 심사로 단일 실행 결과보다 신뢰도가 높지만 독립 재현을 증명하지는
  않는다.
...
당시 인수 조건은 19개 위협과 52개 체크리스트의 스키마 검증, A01·A02 두 번 실행과 A04의
명시적 미실행 사유, 증거 심사 후 한 개 legacy Finding과 두 개 Worker 증적, KISA JSON·Markdown
산출물, 그리고 전체 정적·동적 테스트 통과다.
