...
뜻하지는 않는다. Artifact repository와 server-owned source admission, 새 identity를 사용하는
retry 발행, durable budget/rate permit, Replay executor 연결, typed server-side artifact
finalization과 result-digest 검증, Gate, 실제 PostgreSQL migration/locking acceptance는 남아
있다. 이 경계가 모두 완료되기 전에는 Control Plane이 완전한 durable Replay orchestration을
제공한다고 주장할 수 없다.
