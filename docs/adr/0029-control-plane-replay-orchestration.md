...
binding을 만들지 않고 fail closed한다. 이 내부 service path는 public Replay/admission API를 열지
않는다. exact KISA item/contract/compilation 파생, 새 identity retry 발행, ticket 발행 전 durable
budget/rate permit, Replay executor 연결, typed server-side artifact finalization과 result-digest
검증과 Gate는 남아 있다. 실제 PostgreSQL schema-v3 acceptance는 깨끗한 임시 database에서
migration/locking, `cp_artifacts` append-only 강제와 exact composite Artifact foreign key를 검증해
완료했다. 나머지 실행 경계가 완료되기 전에는 Control Plane이 완전한 durable Replay
