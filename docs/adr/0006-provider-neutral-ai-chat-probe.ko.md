...
> 이 문서의 Validator는 원본 transcript를 독립적으로 재검산하는 증거 심사 경계다. ADR 0027 이후 제품
> 수준의 Confirmed에는 별도 Restricted Reproducer의 새로운 요청·증적과 Oracle 성공이 필요하다.
...
1. `pajin-ai-target:dev`는 M03·M06·A04 신호를 결정론적으로 재현하도록 의도적으로
   취약하게 만든 개발 표적이다.
