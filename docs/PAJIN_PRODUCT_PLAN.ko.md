...
[`ARCH-001`](rfc/0001-pajin-architecture-v2.ko.md),
...
결정은 [`ADR-0050`](adr/0050-consumed-action-permit-dispatch-claim.ko.md), versioned Capability와
Tool binding 결정은
[`ADR-0051`](adr/0051-versioned-capability-definition-and-tool-binding.ko.md)에 기록하고
code-backed authority set 결정은
[`ADR-0052`](adr/0052-code-backed-capability-authority-set.ko.md), inert deterministic scaffold
결정은 [`ADR-0053`](adr/0053-inert-deterministic-capability-scaffolds.ko.md)에 기록한다.
...
`main@59cf210`으로 커밋됐고 CAP-001은 `49b0ebb`, CAP-002는 `84c8cd2`로 커밋됐다.
CAP-003 개발의 부모 기준선은 `main@84c8cd2`이며 `origin/main@84c8cd2`와 동기화돼 있다.
Linux CI 확인은 남아 있다. [`CAP-001`](capability/CAP-001-versioned-capability-definition.ko.md)은 explicit
Capability metadata, canonical full ToolSpec digest, exact-version Registry와 GRAPH-006
`definitionDigest` binding을 추가했다.
[`CAP-002`](capability/CAP-002-metadata-code-backed-authority-interfaces.ko.md)는 Materializer,
...
[`CAP-003`](capability/CAP-003-capability-authoring-sdk-scaffold.ko.md)은 strict-JSON authoring
spec, 일곱 abstract role template, deterministic metadata/schema·benchmark·negative-test·문서
generator, per-file digest, write-last manifest와 write-once CLI를 추가한다. incomplete stub는
인스턴스화할 수 없고 생성 결과는 자동 등록되지 않는다.

## 2026-07-25 B2.8g 구현 상태

[`ADR-0045`](adr/0045-resumable-multipart-portable-artifact-transport.ko.md)에 따라 기존
2 MiB inline 한도를 넘는 portable Artifact는 Control Plane이 관리하는 로컬 object-store의
재개 가능한 multipart 경로를 사용한다. Worker는 최종 manifest와 Executor attestation을
먼저 제출하고, Control Plane이 live replay authority와 서명을 검증한 뒤에만 1 MiB 고정
part를 받는다. 동일 part의 동일 바이트 재전송은 멱등이고 다른 바이트 재전송은 거부한다.
...
[`ADR-0044`](adr/0044-target-signed-tls-session-binding.ko.md)에 따라 signed registry v4의
...
정본인 `docs/PAJIN_PRODUCT_PLAN.md`는 PAJIN의 제품 목표, 범위, 용어와 불변 품질 기준을 정의하는
최상위 기준선이다. 이 한국어 현지화본은 정본을 반영하며, 두 문서가 다르면 정본이 우선한다.
한국어 현지화 문서 사이에 충돌이 있으면 다음 순서로 해석한다.

1. `docs/PAJIN_PRODUCT_PLAN.ko.md` — 현지화된 제품 불변 원칙과 수용 기준
...
4. `docs/KISA_TRACEABILITY.ko.md` — KISA 요구사항과 구현 증적의 연결 상태
5. `README.ko.md` — 현재 코드의 실행 방법, 지원 범위와 알려진 구현 격차

ADR은 제품 기획서의 불변 원칙을 구체화할 수 있지만 암묵적으로 완화할 수 없다. 불변 원칙을
바꾸려면 정본 기획서를 먼저 개정하고, 변경 사유와 이행 영향을 새 ADR에 기록한 뒤 구현해야
한다. 코드나 README의 현재 동작이 기준선과 다르면 이를 새로운 기획으로 간주하지 않고
명시적인 구현 격차로 관리한다.

### 0.1 고정된 Finding 검증 원칙

다음 원칙은 [`ADR-0027`](adr/0027-independent-reproduction-confirmation-boundary.ko.md)이
...
않는다. Control Plane replay-ticket orchestration은 M6-07B로 분리한다.
[`ADR-0029`](adr/0029-control-plane-replay-orchestration.ko.md)는 2026-07-17에 승인되었고, 확정된
경계에 따른 첫 권위 상태 조각은 버전형 Replay 집합체 스키마, 엄격한 시작 검증을 포함한 저장소
...
record이자 proof로 영속화했다. 저장된 5분 Grant는 pending 중 만료될 수 있으므로 이후 실행 권한으로
절대 재사용하면 안 된다. schema v4의 각 append-only row는 고유한 `compilation_id`, Replay
...
2026-07-18 구현된 M6-07B-2C durable issuance는 schema v5에 durable budget account와 reservation,
보수적인 sealed-rate account와 reservation, exact ticket FK를 추가했다. 내부 멱등
...
append한다. 2026-07-22 schema-v11은 append-only `cp_replay_projections`, 서버 소유 source copy,
DB lock 밖의 전체 finalized receipt 재검증과 공통 Gate, source root·batch CAS·정렬된 finalization 집합을
재확인하는 짧은 publication transaction, 역할 기반 projection 조회 API를 구현했다. Finalized item은
배치 전체가 준비될 때까지 `verified`에 머물고, publication commit만 모든 item과 batch를
`gated`/`completed`로 바꾼다. 같은 날 schema-v12는 append-only `cp_replay_retest_sources`로 confirmed
baseline과 부모 Retest Artifact를 1:1 결박한다. 부모 locator가 있는 batch는 서버가
`remediation-retest` 계약·context를 파생하고 부모 Retest의 budget/rate capacity를 사용하며, 모든 음성
receipt와 부모 정상 기능 회귀를 다시 검증해 부모 Retest Run 기반 `kisa-retest.json` projection을
발행한다. 독립 remediation attestation이 없으므로 방어 응답은 `fixed`가 아니라 fail-closed
...
제공하지 않는다. 상세 안전 경계와 재현 명령은 저장소 `README.ko.md`, KISA 커버리지는
`docs/KISA_TRACEABILITY.ko.md`, 확정된 기술 결정은 `docs/adr/`를 기준으로 한다.
...
| 상태 | 의미 | 진입 조건 |
| --- | --- | --- |
| 초안 (`Draft`) | 캠페인 초안 | 대상과 목적 생성 |
| 범위·ROE 검토 (`Scope & ROE Review`) | 범위 및 교전 규칙 검토 | 필수 항목 입력 완료 |
| 권한 부여 (`Authorized`) | 실행 권한 확보 | 승인 주체와 증빙 등록 |
| 계획 수립 (`Planning`) | 에이전트가 공격 계획 구성 | 정책 검증 통과 |
| 실행 중 (`Running`) | 도구와 시나리오 실행 중 | 예산·워커 확보 |
| 검증 중 (`Validating`) | 후보 Finding 검증 | 후보 결과 존재 |
| 보고 중 (`Reporting`) | 결과와 잔여 위험 정리 | 검증 단계 완료 |
| 수정 조치 (`Remediation`) | 개선 작업 추적 | 보고서 승인 |
| 재테스트 (`Retesting`) | 동일·변형 공격 및 회귀 테스트 | 수정 배포 완료 |
| 완료 (`Completed`) | 캠페인 종료 | 종료 기준 만족 |
| 일시 중지 (`Paused`) | 사용자·정책·시스템에 의한 일시 중단 | 재개 가능 |
| 중단 (`Aborted`) | 비상 중단 또는 승인 철회 | 실행 권한 폐기 |
...
```mermaid
flowchart TB
    UI["CLI / Web UI / API"] --> CP["캠페인 Control Plane"]
    CP --> ORCH["에이전트 오케스트레이터"]
    CP --> POLICY["정책·권한 엔진"]
    CP --> EVAL["평가·Finding 서비스"]

    ORCH --> REG["에이전트 / Skill 레지스트리"]
    ORCH --> BUS["작업·이벤트 버스"]
    ORCH --> MEM["Campaign 메모리"]

    BUS --> BROKER["실행 브로커"]
    POLICY --> BROKER
    BROKER --> W1["격리 Worker A"]
    BROKER --> W2["격리 Worker B"]
    BROKER --> W3["브라우저 Worker"]
    ...
    TG --> CLI["보안 CLI 도구"]
    TG --> API["외부 API"]
    TG --> CODE["익스플로잇 / 코드 샌드박스"]

    BROKER --> EV["증적·Artifact 저장소"]
    BUS --> AUDIT["Append-only 감사 로그"]
    EVAL --> FIND["Finding 그래프"]
    EV --> EVAL
    FIND --> REPORT["보고·재테스트 엔진"]
```
...
[`ADR-0002`](adr/0002-tool-gateway-and-worker-isolation.ko.md)를 따른다.
...
```mermaid
stateDiagram-v2
    state "후보" as Candidate
    state "의미 검토" as SemanticReview
    state "검토 필요" as NeedsReview
    state "객관적 기준으로 기각" as RejectedObjective
    state "재현 중" as Reproducing
    state "확정" as Confirmed
    state "결론 불가" as Inconclusive
    state "보고됨" as Reported
    state "수정 중" as Remediating
    state "재테스트 중" as Retesting
    state "종료" as Closed
    state "위험 수용" as AcceptedRisk
    [*] --> Candidate
    ...
    Retesting --> Closed: 검증된 반증 결과(ReplayOutcome)
    Retesting --> Confirmed: 검증된 지지 결과(ReplayOutcome)
...
[`ADR-0025`](adr/0025-candidate-validation-ledger-and-replay-boundary.ko.md)를 따른다. Stage 1은
legacy Validator가 반환한 Finding을 Candidate로 보존하고 Candidate별 Decision snapshot을
구현했다. [`ADR-0026`](adr/0026-trusted-kisa-candidate-admission.ko.md)은 KISA
`ai.chat-probe`의 카탈로그, typed 요청, 실행 identity와 실제 transcript를 재검산하는 trusted
Candidate Producer를 추가했다.

[`ADR-0030`](adr/0030-candidate-aware-atomic-claim-validation.ko.md)은 Provider Validator가
Finding을 재작성하지 않고 exact Candidate ID·digest에 결박된 validity·impact·severity Atomic
Claim별 `supports`·`contradicts`·`insufficient` Decision만 반환하도록 제한한다.
[`ADR-0031`](adr/0031-blind-evidence-review-boundary.ko.md)의 별도 Blind Evidence Reviewer는
Candidate identity·disposition·severity·기존 Decision을 제거한 validity와 선택적 impact Packet만
받고, 결정론적 Reconciler는 `corroborated`·`contested`·`inconclusive`를 Candidate 상태 변경 없이
기록한다.

[`ADR-0032`](adr/0032-fresh-capability-validation-controls.ko.md)와
[`ADR-0033`](adr/0033-registered-validation-control-materializers.ko.md)의 B2.2 확장은 KISA
M03·M06·A04 validity Claim에 대해 별도 opt-in Control Executor를 제공한다. 코드 등록
materializer의 ID·version·scenario digest를 Plan v1alpha2에 봉인하고, Baseline·Negative
Control·Counterfactual마다 fresh non-delegable `max_calls=1` Capability와 고유
request·session·evidence·receipt를 만든다. M03·M06은 benign `READY` Counterfactual을
사용하고 A04는 두 번째 memory query를 유지한 채 첫 poison write만 바꾼다. 결과는 항상 정보
전용이며 Candidate disposition·severity·confirmation eligibility를 변경하지 않는다.

[`ADR-0034`](adr/0034-diverse-independent-severity-review.ko.md)의 B2.3 첫 수직 조각은
...
독립 도출을 `corroborated`·`contested`·`inconclusive`로 비교하지만 항상 정보 전용이며
...
[`ADR-0035`](adr/0035-claim-replay-public-state-projection.ko.md)의 B2.4 첫 수직 조각은 기존
...
[`ADR-0036`](adr/0036-claim-bound-replay-execution-authority.ko.md)의 B2.5 첫 수직 조각은
...
[`ADR-0037`](adr/0037-control-plane-claim-specific-public-projection.ko.md)의 B2.6 첫 수직
조각은 이 Claim별 실행 권위를 PostgreSQL Control Plane 끝까지 보존한다. 명시적
...
[`ADR-0038`](adr/0038-portable-claim-receipt-attestation.ko.md)의 B2.7 첫 수직 조각은 명시적
...
[`ADR-0039`](adr/0039-executor-attested-portable-artifact-transport.ko.md)의 B2.8a 첫 수직
조각은 별도 executor workload Ed25519 key와 Control Plane 공개키 trust anchor를 분리한다.
executor statement는 exact batch·item·Job·ticket·fence·compilation·execution context,
canonical permit set과 Replay request IDs, sealed output digest와 두 seal root를 서명한다.
raw 2 MiB·file당 1 MiB·256 files·depth 24로 제한된 content-addressed bundle은 공유 volume
없이 opaque server-owned staging reservation으로 전송되며, Control Plane은 bytes를 복사하기
전에 서명과 발급 권위를 검증하고 기존 managed Artifact admission으로 tree·Run·receipt·seal을
다시 검증한다. transport와 attestation digest는 finalization 및 projection input authority에
결박되고 전체 attestation은 projection seal 안에 보존된다. 이는 executor 관찰 증명이지 target
issuer 증명은 아니므로 `needs-review` 상한은 유지하며, B2.8b target-side signed receipt가
다음 독립 실행 경계다.

[`ADR-0040`](adr/0040-target-issued-challenge-bound-replay-receipts.ko.md)의 B2.8b 수직
조각은 명시적 `target_attestation` opt-in에 `pajin.kisa-target-attestation:v4` 정책을
부여한다. Control Plane은 durable permit에서 최대 30초의 challenge를 파생하고, Target은
별도 Ed25519 key로 exact request와 receipt 제외 response digest를 서명한다. host proxy는
receipt를 포함한 응답을 관찰하고 executor는 Target receipt와 proxy receipt의 결합을 기존
statement 안에서 서명한다. Control Plane은 외부 bytes 복사 전 executor authority를, managed
import 후 permit→challenge→transcript→Target signature→proxy observation 결합을 다시
검증한다. 이 exact validity Claim만 `VERIFIED_INDEPENDENT_REPLAY`로 승격할 수 있으며
contradiction·inconclusive 경로는 그대로 우선한다. 현재 첫 조각은 평문 HTTP와 단일 Target
trust anchor에 한정되므로, HTTPS 관찰과 다중 Target registry는 후속 경계다.

이 단계들 자체는 Candidate admission과 원 증거 심사만 강화한다.
[`ADR-0027`](adr/0027-independent-reproduction-confirmation-boundary.ko.md)에 따라 Semantic
...
[`ADR-0028`](adr/0028-durable-local-replay-ticket-ledger.ko.md)에 따라 로컬 KISA positive와
...
- 차단·거부율 (Block / Refusal Rate)
- 과잉 거부율 (Over-refusal Rate)
- 재현 성공률 (Reproducibility Rate)
- 민감 데이터 노출 건수 (Sensitive Data Exposure Count)
- 무단 도구 호출 건수 (Unauthorized Tool Invocation Count)
- 침해 성공까지의 평균 대화 턴 수 (Mean Turns to Compromise)
- 토큰·비용 증폭률 (Token / Cost Amplification)
- 지연 시간 및 자원 성능 저하 (Latency and Resource Degradation)
- Judge 일치율 (Judge Agreement Rate)
...
[`ADR-0001`](adr/0001-agent-runtime-and-orchestration.ko.md)에서 확정하였다.
...
상태 조회 API와 permit 0개 fresh-identity retry 발행도 구현됐다. Schema-v11은 모든 item finalization을
...
schema-v14 anti-rollback·제한된 pin rotation, registry v4 TLS 1.2 양측 session binding이
구현됐다. TLS 1.3 RFC 9266 exporter와 대형 object-store/multipart 전송은 별도 완료 기준으로
남아 있다.
...
### 20.5 M6-06 로컬 영속형 Replay Ticket 완료 기준
...
### 20.6 M6-07 로컬·Control Plane Replay 오케스트레이션 완료 기준

M6-07은 실행 권한과 영속성 경계가 다른 두 범위로 분리한다.
...
구현은 schema v11의 multi-item positive-confirmation projection publication까지 도달했다. 그 기반은 버전형
...
projection publication은 schema v11의 append-only publication authority, copy-on-project,
source-root/batch-CAS/정렬된 finalization 집합 검증으로 구현됐다. Negative Control Plane retest,
exact Claim별 portable receipt, executor-attested portable Artifact, Target-issued exact exchange,
signed registry와 TLS 1.2 양측 session binding도 구현됐다. 대형 object-store/multipart 전송과
TLS 1.3 RFC 9266 exporter는 남아 있으므로 M6-07B 전체 완료를 주장할 수 없다.
...
- 프로세스 재시작과 여러 Worker에서도 유지되는 영속형 budget/request-rate 상태.
...
- 승인된 ADR-0029의 첫 권위 상태 조각: 버전형 PostgreSQL Control Plane Replay
  batch/item/ticket/event 스키마, 저장소 관리형 v1→v2 마이그레이션, 엄격한 내부 payload,
...
- Schema-v9 exact-KISA 실행/finalization: 전용 Replay Worker principal·daemon, dispatch 직전 동일 permit
  ...
  terminal failure
- Schema-v11 multi-item projection publication: append-only `cp_replay_projections`, finalization 뒤
  `verified` 대기 상태, 서버 소유 immutable source copy, DB lock 밖의 source/전체 Replay receipt 재검증과
  `validation/v1alpha1` 봉인, source root·batch CAS·정렬된 finalization input authority가 그대로일 때만
  projection Artifact와 모든 item/batch의 `gated`/`completed` 전이를 commit하는 멱등 publisher,
  `/v1/replay/batches/{batch_id}/projection` 역할 기반 조회
- Schema-v12 negative Control Plane retest: append-only `cp_replay_retest_sources`의 baseline/부모
  Retest 1:1 authority, 두 managed seal의 exact 재검증, 부모 Retest budget/rate capacity, compilation별
  `ReplayRetestContext`, projection-input v2의 dual-source CAS, 전체 음성 receipt와 정상 기능 회귀를
  `KISARetestService`로 다시 판정해 부모 Retest Run 위에 봉인하는 `kisa-retest.json` projection
...
실행 경계와 기술 구조는 모두 승인된 ADR-0001부터 ADR-0044까지에 기록돼 있다. ADR-0029는
...
receipt, HTTPS CONNECT·leaf SPKI 결박, signed registry v3의 schema-v14 anti-rollback·제한된
pin rotation과 registry v4 TLS 1.2 양측 session binding도 구현됐다. TLS 1.3 RFC 9266
exporter와 대형 object-store/multipart Artifact 전송이 남아 있어 M6-07B 전체는 미완료다.
다음 항목은 Phase 3-4의 후속 작업 전에
추가 결정이 필요하다.
...
현재 한국어 현지화 문서군은 다음과 같다. 문서 권한은 0장을 따르며, 제품 정본은
`docs/PAJIN_PRODUCT_PLAN.md`다.

1. `README.ko.md` — 설치, 실행, 안전 경계, Mode Pack과 Control Plane 운영 계약
2. `docs/PAJIN_PRODUCT_PLAN.ko.md` — 제품 방향, 요구사항, 현재 기준선과 로드맵
3. `docs/KISA_TRACEABILITY.ko.md` — KISA 요구사항, 코드, 증적, 실행 커버리지 연결
4. ADR-0001부터 ADR-0044까지 — 승인된 런타임·정책·Mode Pack·Control Plane, Candidate-aware Atomic Claim Validator, Blind Evidence 독립 검토, 등록형 Validation Control, 다양한 Provider/model 기반 독립 severity, replay orchestration, Claim별 실행 권위·portable attestation, Target registry와 TLS session binding 의사결정
