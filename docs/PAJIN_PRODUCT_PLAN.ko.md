...
[`ADR-0032`](adr/0032-fresh-capability-validation-controls.ko.md)의 B2.2 첫 수직 조각은 KISA M03
validity Claim에 대해 별도 opt-in Control Executor를 제공한다. Baseline·Negative
Control·Counterfactual마다 fresh non-delegable `max_calls=1` Capability와 고유
request·session·evidence·receipt를 만들고 별도 Control Run에 봉인한다. 결과는 항상 정보 전용이며
Candidate disposition·severity·confirmation eligibility를 변경하지 않는다. M03 밖의 Control,
독립 severity, Provider/model 다양성과 portable attestation은 후속 범위다.
...
실행 경계와 기술 구조는 모두 승인된 ADR-0001부터 ADR-0032까지에 기록돼 있다. ADR-0029는
...
4. ADR-0001부터 ADR-0032까지 — 승인된 런타임·정책·Mode Pack·Control Plane, Candidate-aware Atomic Claim Validator, Blind Evidence 독립 검토, Validation Control과 replay orchestration 의사결정
