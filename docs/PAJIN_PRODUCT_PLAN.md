...
opaque locator를 통한 완료·봉인 source의 내부 server-owned admission을 추가했다. sealed-source
KISA 파생, 새 identity retry 발행, ticket 발행 전 durable budget/rate permit,
executor/finalization/Gate 연결이 남아 있어 M6-07B 전체는 미완료다. 실제 PostgreSQL schema-v3
acceptance는 깨끗한 임시 database에서 migration/locking, `cp_artifacts` append-only 강제와 exact
composite Artifact foreign key를 검증해 완료했다.
...
| Control Plane | 초기 구현 | FastAPI, PostgreSQL Job queue, 승인 체크포인트, fence형 취소, lease·heartbeat, 단일 Worker daemon; ADR 0029 Replay authority state, M6-07B-2A managed Artifact admission 및 immutable schema-v3 metadata, migration/locking·`cp_artifacts` append-only·exact composite Artifact FK의 live PostgreSQL acceptance는 구현했으며 KISA 파생·retry·ticket 발행 전 durable permit·executor/finalization/Gate는 미완료 |
...
두 가지 제한된 기반을 구현했다. 첫 조각은 versioned Replay aggregate, repository-managed v1→v2
...
실제 PostgreSQL schema-v3 acceptance는 깨끗한 임시 database에서 완료됐고 forward
...
연결이 남아 있어 전체 완료를 주장할 수 없다. Accepted ADR은 최소한 다음을 결정한다.
...
ADR-0029의 첫 M6-07B authority-state 조각과 M6-07B-2A managed Artifact admission 및 immutable
schema-v3 metadata는 구현됐다. 다만 sealed-source KISA 파생, retry 발행, ticket 발행 전 durable
permit과 executor/finalization/Gate 연결이 남아 있어 M6-07B 전체는 미완료다. 실제 PostgreSQL
v3 migration/locking, `cp_artifacts` append-only와 exact composite-FK acceptance는 완료됐다. 다음
