...
있으므로 전체 완료를 주장할 수 없다. Accepted ADR은 최소한 다음을 결정한다.
...
공통 Gate도 구현됐고 Compose는 별도 Replay Worker credential로 daemon을 활성화한다. Public Replay
admission/read API, 자동 fresh-identity retry 발행, multi-item versioned projection publication과 negative
Control Plane retest가 남아 있어 M6-07B 전체는 미완료다. 다음 항목은 Phase 3-4 진행 전에 추가
결정이 필요하다.
