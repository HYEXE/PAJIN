...
있어 M6-07B 전체는 미완료다. Portable/off-host 서명 proof, 다른 Mode의 materializer·Oracle과
구조화 협업 메모리는 후속 과제다.
...
구현됐다. 다만 public Replay admission/read API, actual executor/pre-dispatch permit-use 집행, exact
Campaign execution-context bundle, 새 identity retry 발행,
typed finalization/Gate 연결과 negative Control Plane retest가 남아 있어 M6-07B 전체는
