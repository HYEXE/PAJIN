...
[`ADR-0049`](adr/0049-durable-single-campaign-sqlite-graph-store.md)에 기록한다.
...
single-Campaign SQLite store를 추가했다. 로컬 `main@827404c`에는 GRAPH-004가 커밋됐고 원격
`main`은 `42dd0a0`이며 GRAPH-005는 로컬 검증 완료, Linux CI 확인은 남아 있다.
