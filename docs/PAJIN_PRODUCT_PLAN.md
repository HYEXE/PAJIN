...
[`ADR-0052`](adr/0052-code-backed-capability-authority-set.md), inert deterministic scaffold
결정은 [`ADR-0053`](adr/0053-inert-deterministic-capability-scaffolds.md)에 기록한다.
...
preflight를 추가했다.
[`GRAPH-005`](graph/GRAPH-005-durable-sqlite-graph-store.md)는 Event Log, append-only Projection
history, immutable Snapshot, cross-process host-local CAS, reopen reconciliation을 위한 별도
...
Linux CI 확인은 남아 있다. [`CAP-001`](capability/CAP-001-versioned-capability-definition.md)은 explicit
...
[`CAP-003`](capability/CAP-003-capability-authoring-sdk-scaffold.md)은 strict-JSON authoring
...
[`ADR-0039`](adr/0039-executor-attested-portable-artifact-transport.md)의 B2.8a 첫 수직 조각은
별도 executor workload Ed25519 key와 Control Plane 공개키 trust anchor를 분리한다. executor
statement는 exact batch·item·Job·ticket·fence·compilation·execution context, canonical permit
set과 Replay request IDs, sealed output digest와 두 seal root를 서명한다. raw 2 MiB·file당
1 MiB·256 files·depth 24로 제한된 content-addressed bundle은 공유 volume 없이 opaque
server-owned staging reservation으로 전송되며, Control Plane은 bytes를 복사하기 전에 서명과
발급 권위를 검증하고 기존 managed Artifact admission으로 tree·Run·receipt·seal을 다시 검증한다.
transport와 attestation digest는 finalization 및 projection input authority에 결박되고 전체
attestation은 projection seal 안에 보존된다. 이는 executor 관찰 증명이지 target issuer
