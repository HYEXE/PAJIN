...
[`ADR-0042`](adr/0042-worker-observed-tls-leaf-spki-binding.md)에 따라 HTTPS
...
negative Control Plane retest, Claim별 public projection, Ed25519 portable receipt proof와
별도 executor workload key 기반 bounded multi-host Artifact 전송이 구현됐다. target issuer와
대형 object-store/multipart 전송은 별도 완료 기준으로 남아 있다.
...
별도 executor workload key가 exact permit·sealed output과 bounded portable Artifact를 서명해
다른 host의 Control Plane으로 전달하는 B2.8a도 구현됐다. target issuer와 대형
object-store/multipart Artifact 전송이 남아 있어 M6-07B 전체는 미완료다.
