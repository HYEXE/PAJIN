...
> `kisa-retest.json`을 봉인한다. Portable/off-host proof는 후속이므로 M6-07B 전체는 아직 미완료다.
...
- M6-07A의 명시적 Local KISA coordinator는 exact M03·M06·A04 allowlist와 한 프로세스·한
  ...
  admission/read API, fresh-identity retry 발행, schema-v11 multi-item projection과 schema-v12 dual-source
  negative Control Plane retest는 구현됐다. Portable/off-host proof는 남아 있다.
...
[ADR 0034](adr/0034-diverse-independent-severity-review.ko.md)을 따른다.
