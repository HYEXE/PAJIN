...
AI runtime exit gate without externally issued releases and an actual sealed Campaign.
...
## Verified rollout input

CAP-006 can also measure an `ExistingModeCapabilityRollout` admitted from seven externally
reviewed CAP-004 release bundles. The rollout path contributes the seven code-authored benchmark
mappings and seven verified lifecycle heads, so its reference measurement has:

- definition, complete authority, benchmark mapping, Success Oracle authority, Replay support,
  and signed lifecycle coverage at their full denominators; and
- 17 remaining operational gaps: seven delivery records, seven Oracle observations, and three
  Replay observations.

This is a verifier contract and deterministic test fixture, not a claim that organization-issued
production releases or real benchmark Runs exist. Private keys, review decisions, and signatures
are never generated by the rollout API.

For the Web + AI runtime exit gate, `CapabilityOperationalEvidenceSet` packages the exact rollout
inputs and binds them to the release-set digest. The gate reloads that set from a fully sealed Run,
requires every `sourceDigest` and `evidenceDigest` to match another sealed artifact SHA-256, and
then calls the same rollout metric collector. It never trusts a submitted metrics report or a
submitted `complete` flag. A complete rebuilt report means the explicit measurement gaps are
closed; it does not convert CAP-006 into an unapproved score threshold.
...
- foreign, duplicate, authority-drifted, unmapped Oracle, and unsupported Replay rejection;
- current CAP-005 baseline counts and gaps;
- exact seven-release admission and rollout-metric aggregation;
- release/trust-key order independence; and
- missing/duplicate release, substituted signing authority, release-set tamper, and mapping-drift
  rejection;
- sealed operational-evidence source binding and report rebuilding for the Web + AI gate; and
- missing sealed source, unsuccessful dispatch, activation expansion, and post-seal mutation
  rejection.

## Follow-up boundaries

- organization-issued reviewed CAP-004 release bundles for the seven CAP-005 adapters;
- delivery timestamps from the real contribution workflow;
...
- the opt-in GRAPH-006/Gateway bridge and sealed exit-gate verifier are implemented; an
  organization-issued Web + AI Campaign run and its sealed operational observations remain
  required.
