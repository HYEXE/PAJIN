...
SUP-007A supplies an explicit direct-call T0/T1 no-write composition over the deployment-owned
Grant, managed Run root, Gateway, Worker, existing Permit, and authenticated outcome gates. SUP-007B
exposes it through the existing Control Plane Campaign Job as `general-attack-v1`, exact-rebuilding
the Proposal and intent against the SHA-256-pinned Capability Graph deployment. The first profile is
approval-free, non-networked, and zero-cost. T2, T3+, write, cleanup-required, caller-selected Run,
