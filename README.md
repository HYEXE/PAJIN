...
| Capability registry | Immutable definition and exact Tool binding, complete seven-role code authority sets, inert deterministic authoring scaffolds, an additive signed lifecycle registry, an explicit opt-in seven-Capability compatibility bundle, and content-addressed CAP-006 Registry/lead-time/Oracle/Replay/lifecycle measurement over an external denominator. The closed adapter inventory has seven exact benchmark mappings and admits only a complete externally reviewed seven-release set; it never generates keys or approvals. An additive opt-in bridge can now activate an explicit signed subset, compile exact CAP-002 requests, expose only that subset to GRAPH-006, and enter the existing Tool Gateway from the Permit's first-consumption callback. A local Web + AI fixture verifies the structural gate. All adapters remain experimental and no existing Mode path auto-activates them; durable Registry storage, explicit Permit/result audit linkage, organization-issued releases, and sealed operational evidence remain follow-up work. |
