...
| Discovery and orchestration contracts | Versioned `SurfaceObservation`, `AttackSurface`, and `AttackSurfaceSet` artifacts; canonical HTTP-operation, non-executable OpenAPI route-template, authentication-boundary, file-upload-boundary, explicit RAG-boundary, registered MCP server/resource/resource-template/prompt/tool, and schema-bound Tool-interface locators; deterministic domain-separated identities; exact request/result/evidence/root lineage; bounded canonical JSON; and fail-closed ordering, uniqueness, and lineage validation. DISC-001 adds a code-owned `DiscoveryAdapter` protocol and exact ID/version/digest Registry that binds implementation context, trusted-network-receipt requirements, and the full Tool contract, rejects duplicate selection, secret-like context, and live adapter/Tool drift, and records the selected adapter reference in admission authority and projection audit. DISC-002 and DISC-003A/B/C provide the cumulative HTTP/OpenAPI route, authentication, file-upload, and explicit RAG chain with sealed receipt revalidation and no `$ref`, YAML, crawler, content, credential, or destination expansion. DISC-003D adds a separate T0 registered MCP discovery Tool: its fixed Worker action accepts only a sealed server ID, bounds capability-aware pagination, and publishes only protocol/capability identity, portable names, required flags, URI schemes, and SHA-256 URI/template/schema digests. It retains no process commands, descriptions, annotations, raw schemas/URIs, resource content, or prompt content/values and grants no discovered execution authority. The legacy producer remains compatible, all adapters use exact Registry references, and sealed admission/projection integration is covered. A3 adds an explicit opt-in single-call Recon wave. A4 re-verifies the sealed projection, compiles versioned Hypotheses through code-registered rules, and runs one fresh-Capability Specialist wave. A5 adds a separate explicit opt-in bounded replanning control Run with append-only observations and one novel transition into a second fresh-Capability wave. ORCH-001 adds an additive `SurfaceSnapshotAuthority` plus canonical Plan/Task digests, persists the bound Plan, and re-verifies the exact sealed projection before capability issuance and every Tool dispatch. ORCH-002 adds a content-addressed Campaign/Snapshot/Compiler/Observation/transition authority and an opt-in third wave, re-verifies every sealed ORCH-001 Plan, selects transitions only from the current wave, and blocks repeated state, cycles, and Scope/Snapshot replacement before further dispatch. WALK-001 adds a Campaign-bound `HTTPFileUploadReconPlanner`, binds its request to the exact DISC-003B adapter, and publishes only when trusted admission contains an `http-file-upload` Surface. WALK-002 adds an exact DISC-003C Recon path and a sealed, content-addressed, non-executable RAG-injection Hypothesis bound to the complete Campaign digest, ORCH-001 Snapshot, registered rule, and co-located corpus-ingest plus file-upload Surfaces. WALK-003 binds the complete sealed WALK-002 lineage to a separate exact DISC-003D Snapshot, MCP server/tool Surface and schema, immutable Capability definition, registered ToolSpec and remote identity, plus independent-user-approval rule. Its `registered-not-authorized` artifact creates no activation, Grant, Permit, request, argument, or dispatch. WALK-004 re-verifies that sealed authority, admits its exact state into an immutable typed Graph, and selects only a `proposed-not-authorized` independent-approval request Plan. Content-addressed evidence, expected-state comparison, and bounded state history reject source substitution, stale state, repeated state, cycles, and Campaign/Snapshot/Capability expansion without creating a Grant, Permit, request, argument, or dispatch. WALK-005A reopens that Plan and a separate sealed execution Run, requires an approval receipt recorded before a consumed ActionPermit dispatch, reconciles the exact successful Gateway lifecycle and target-provided authorization/data-access observables, and emits only an unconfirmed Candidate plus deterministic validity/impact/severity Atomic Claims. It does not infer those observables from suspicious input or reuse the KISA-only Replay path. WALK-005B1 seals the exact validity Claim, source publication provenance, original request semantics, and seven freshness requirements into a non-executable Replay Plan. WALK-005B2 requires that Plan's approval receipt before a fresh Permit claim, re-verifies the successful Gateway evidence, rejects all seven original execution identities, and publishes only a reproduced validity projection with confirmation eligibility fixed to false. WALK-005C1 reopens that authority, adopts only the Plan-bound fresh validity replay as the MCP confirmation basis, keeps impact and severity source-bound and information-only, and seals the validated Finding, typed report plus exact Markdown, and a non-executable remediation baseline. WALK-005C2 accepts only another post-baseline B2 authority with the same Plan and Claim but seven fresh execution identities, binds both publication roots, and publishes a sealed `still-vulnerable` Retest with fixed eligibility false, remediation application unattested, and regression not measured. Existing two-wave construction, Hypothesis v1alpha1 wire shapes, legacy Recon/Replanning readers, and non-opt-in paths remain readable. Shared Campaign agent, Tool-call, cost, time, and rate limits still apply. WALK-006 records a code-owned snapshot-only Shadow policy's human remediation-review Task and autonomous Stop/escalation Decision without mutating the baseline or granting execution. BENCH-003A binds that record to a baseline-only BENCH-001 Manifest and publishes only a structural Decision delta; all twelve metric names remain present but values and deltas are absent, and benchmark comparison plus activation eligibility are false. Multi-adapter scheduling, BENCH-003B measured Results, and Phase 6 model-backed Supervisor work remain future work. |
...
Discovery uses a separate `mcp-discover` Worker action. It accepts only the registered server ID,
enumerates advertised MCP interfaces under fixed item/page limits, and returns digest-only
resource, template, and schema identities plus non-value prompt argument declarations. It never
reads a resource, resolves a template, gets a prompt, or calls a discovered tool.
