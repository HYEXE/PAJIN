...
| Canonical Graph | Single-Campaign append-only SQLite Event, Projection, Snapshot, and consumed ActionPermit authority with exact retry, stale-decision rejection, crash reconciliation, a content-addressed verified backup manifest, restore to a new path only, and subprocess hard-exit coverage for committed, uncommitted, backup-publication, Permit-commit, RunStore-claim, and external Gateway-side-effect boundaries. The backup is host-local and self-consistent; off-host retention, encryption, external signatures/anchors, HA, and exhaustive power-loss injection remain follow-up work. |
