...
| Capability registry | Immutable definition and exact Tool binding, complete seven-role code authority sets, inert deterministic authoring scaffolds, an additive signed lifecycle registry, an explicit opt-in seven-Capability compatibility bundle, and content-addressed CAP-006 Registry/lead-time/Oracle/Replay/lifecycle measurement over an external denominator. The closed adapter inventory now has seven exact benchmark mappings and can admit only a complete externally reviewed seven-release set while binding each signed bundle and mapping digest; it never generates keys or approvals. Without those external releases and operational samples, the honest project baseline remains incomplete. All adapters remain experimental and are neither auto-activated nor runtime-wired; durable storage, operational evidence, organization-issued releases, and dispatch remain follow-up work. |
