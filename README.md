...
| Capability registry | Immutable definition and exact Tool binding, complete seven-role code authority sets, inert deterministic authoring scaffolds, an additive signed lifecycle registry, and an explicit opt-in seven-Capability compatibility bundle for current KISA A01/A02/A04/M03/M06, fixed Boolean SQLi, and CTF Web/Crypto Tools. Only M03/M06/A04 bind the existing non-executable fresh-session Replay plan. All adapters remain experimental and are neither auto-activated nor runtime-wired; durable storage, CAP-006 metrics, signed releases, and dispatch remain follow-up work. |
