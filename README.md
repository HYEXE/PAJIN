...
| AI M03 measured system-prompt disclosure | Implemented, bounded source and floor | [AI-002A](docs/benchmark/AI-002A-exact-m03-measured-case-authority.md) registers one neutral public M03 case, private Ground Truth, immutable Target/Worker/proxy contracts, and the canonical six-operation DOMAIN-006 floor. [AI-002B](docs/benchmark/AI-002B-registry-governed-disposable-m03-source-measurement.md) executes the fresh internal source. [AI-002C](docs/benchmark/AI-002C-independent-fresh-session-replay-controls-ai-floor.md) adds two fresh-session Replay operations, exact Baseline/Negative/Counterfactual Controls, disjoint execution identities, measured request/Tool/zero-cost accounting, mandatory cleanup, and a public-safe fourteen-metric floor. Product, Graph/Finding/report/delivery, credentials, external targets/providers, M06, A04, MCP, RAG, arbitrary prompts, general AI authority, and exact-clean real-Docker conformance remain unavailable under [ADR-0259](docs/adr/0259-select-governed-measured-ai-system-prompt-disclosure-after-phase-24.md). |
...
| Product projection | Implemented, bounded; conformance verified | UX-008 reopens the sealed REDTEAM-002 aggregate and every source into profile-bound, non-confirming product sections. UX-009A separately seals one exact WEB-002D authority as measured-case Scope, content-free Evidence references, fourteen public floor metrics, a `benchmark-ground-truth-match` claim-ceiling Finding, and unavailable report state. UX-009B adds an immutable deployment registry and zero-argument reader that pins the exact product/source identities and complete private reopen context, then invokes the UX-009A loader on every read. UX-009C exposes that unchanged wire through one body/query-free, non-cacheable Operator-only Control Plane GET and a same-origin strict text-only Console view. UX-009D supplies fresh-spawn deterministic publication/read, isolated integrity-failure, and whole-call side-effect conformance through the existing real-Docker test. Exact-SHA run `33410801762`, job `99549584968`, passed the fresh-spawn test and unconditional Docker-residue audit at commit `6cb58c1cf69795c86a4ccb6614b4e6fdf445ecbf`. Caller-selected paths or verifier inputs, Campaign Scope/Profile inference, raw/private disclosure, Graph mutation, report delivery, and additional execution remain absent. |
...
| Network measured service identification | Implemented, conformance verified | [NET-002A](docs/benchmark/NET-002A-exact-isolated-service-measured-case-authority.md) registers NET-001D's exact five positive banners and one unknown negative Control with separate private Ground Truth, fixed emitter/image contracts, and the exact DOMAIN-006 protocol/floor. [NET-002B](docs/benchmark/NET-002B-registry-governed-disposable-network-source-measurement.md) binds observed OCI IDs and runs one fresh internal no-published-port Target plus one separately approved proxy-only Worker action per case. [NET-002C](docs/benchmark/NET-002C-independent-fresh-worker-network-floor-evaluation.md) runs a globally identity-disjoint six-case Replay, contextfully reopens both sets, evaluates the exact fourteen-metric Network floor, and seals only public-safe lineage and aggregates. [NET-002D](docs/orchestration/NET-002D-bounded-network-measurement-product-read-and-conformance.md) adds a Network-only sealed projection, immutable deployment-pinned zero-argument reader, and body/query-free non-cacheable Operator GET. Its product includes only public case references, aggregate metrics, applicability, floor state, and literal-false authority markers. Commit `9b3d8035252d26334d35caa55c0270356c71683a` passed Ubuntu 24.04 exact-commit real-Docker run `33494188536`, job `99812441408`, including twelve source/Replay executions and the unconditional zero-residue audit. Existing Finding-oriented wires are not reinterpreted, and DNS, UDP, port enumeration, active writes, credentials, external targets, service confirmation, Findings, Graph mutation, reporting, and general scanning remain absent. |
...
Ubuntu workflow completed at exact checkpoint
`6cb58c1cf69795c86a4ccb6614b4e6fdf445ecbf` as run `33410801762`, job `99549584968`. The
fresh-spawn conformance passed in 836.08 seconds and the unconditional PAJIN Docker-residue audit
also succeeded, completing the Phase 23 Exit Gate without widening the product authority boundary.
AI-001A is
...
and unevaluated DOMAIN-006 AI floor without runtime authority.
[AI-002B](docs/benchmark/AI-002B-registry-governed-disposable-m03-source-measurement.md) adds only
the disposable internal Target source lifecycle, exact existing approval/Permit/Gateway/Worker
path, Target-signed receipt, eight pre-dispatch denial probes, private prompt/transcript/runtime
custody, cleanup, and contextful reopen.
[AI-002C](docs/benchmark/AI-002C-independent-fresh-session-replay-controls-ai-floor.md) now adds
only the two independently authorized fresh-session Replay operations, exact three-Control
contrast, six-way disjoint execution identity, measured six request units and Tool calls, measured
zero local-provider cost, mandatory cleanup, separate private custody, and the exact fourteen
DOMAIN-006 observations. It does not confirm the source or create a Graph/Finding/product/report/
delivery authority. The bounded product read and exact-clean Ubuntu real-Docker conformance remain
AI-002D work.
...
numeric benchmark result, or general Network runtime. ADR-0258 now selects the exact six-case
synthetic set as Phase 24's governed measured Network slice. NET-002A implements only the
content-addressed public/private case registration, fixed emitter and immutable image-contract
profiles, DOMAIN-006 protocol, and unevaluated floor. NET-002B implements the six-case source path:
it independently binds observed image IDs, uses a fresh internal no-published-port Target per case,
keeps the Worker proxy-only, consumes one separate Approval and Permit, and seals public lineage
apart from raw private output and cleanup evidence. NET-002C runs a second globally identity-disjoint
six-case lifecycle, reopens both sets and private Ground Truth, and evaluates the exact DOMAIN-006
fourteen-metric floor while keeping raw labels, banners, and runtime identities private. Its passing
floor is synthetic benchmark evidence, not service confirmation or a Finding. NET-002D now
projects only those public-safe cases and aggregates into a sealed Network product, pins its full
verifier context in an immutable deployment registry, and exposes one zero-argument non-cacheable
Operator read. The product and read add no execution, disclosure, Finding, Graph, report, or
delivery authority. Commit `9b3d8035252d26334d35caa55c0270356c71683a` passed the exact-commit
Ubuntu 24.04 real-Docker run `33494188536` with twelve source/Replay executions and unconditional
zero residue. The existing
finding-oriented benchmark wires are not reused as service-classification authority. CLOUD-001A adds a secret-free typed registry
...
PAJIN is licensed under the [Apache License, Version 2.0](LICENSE).
