...
| AI M03 measured system-prompt disclosure | Implemented, bounded source only | [AI-002A](docs/benchmark/AI-002A-exact-m03-measured-case-authority.md) registers one neutral public M03 case, separate private known-positive Ground Truth, immutable Target/Worker/proxy contracts, the canonical six-operation protocol, and the unevaluated DOMAIN-006 floor. [AI-002B](docs/benchmark/AI-002B-registry-governed-disposable-m03-source-measurement.md) executes only its one fresh internal no-published-port vulnerable source through the existing approval, one-use Permit, Gateway, proxy-only Worker, and Target-receipt path; it seals prompt/check/transcript/runtime privately and proves eight caller substitutions fail before dispatch. Replay, benchmark Controls, floor, product, Graph/Finding/report/delivery, credentials, external targets/providers, M06, A04, MCP, RAG, arbitrary prompts, and general AI authority remain unavailable under [ADR-0259](docs/adr/0259-select-governed-measured-ai-system-prompt-disclosure-after-phase-24.md). |
...
custody, cleanup, and contextful reopen. Independent measured Replay, benchmark Controls, floor,
bounded product read, and exact-clean Ubuntu real-Docker conformance remain AI-002C and AI-002D
work.
