...
| Capability registry | Immutable definition and exact Tool binding, complete seven-role code authority sets, inert deterministic authoring scaffolds, an additive signed lifecycle registry, an explicit opt-in seven-Capability compatibility bundle, and content-addressed CAP-006 Registry/lead-time/Oracle/Replay/lifecycle measurement over an external denominator. The current honest baseline is 7/7 definition and authority coverage plus 3/3 Replay support, with benchmark mappings, delivery timestamps, execution observations, and signed releases preserved as gaps. All adapters remain experimental and are neither auto-activated nor runtime-wired; durable storage, operational evidence, signed releases, and dispatch remain follow-up work. |
