...
| Discovery 계약 | 버전형 `SurfaceObservation`, `AttackSurface`, `AttackSurfaceSet` artifact, canonical HTTP operation과 schema-bound Tool interface locator, 도메인 분리 결정론 identity, exact request/result/evidence/root 계보, bounded canonical JSON, ordering·uniqueness·lineage fail-closed 검증을 구현했습니다. Trusted Surface 생산·Scope admission, Recon 통합, Hypothesis planning과 replanning은 아직 구현 전입니다. |
...
| 주요 공백 | Trusted Surface 생산·admission, Recon Wave 통합, Hypothesis·Observation Graph·replanning, multi-host/object-store Artifact 전송, KISA 외 Local replay 오케스트레이션, portable/off-host replay proof, Finding/보고서 검토 UI, 분산 Worker, 외부 연동, 독립적으로 앵커링된 운영 증거 |
