"""EFFECT-003 trust and compatibility tests; no simulated model-quality claim."""
...
from pajin.benchmark.effectiveness_revision.evidence import ComparisonIndex, recompute_comparison
from pajin.benchmark.effectiveness_revision.plan import (
    ComparisonPlan,
    ComparisonSuite,
    comparison_pins,
)
from pajin.benchmark.effectiveness_revision.runner import consume_corpus
from pajin.benchmark.effectiveness_revision.scoring import DetectorInput, measure_pair, verify_pair
from pajin.runtime.store import RunStore


def revision_fixture() -> ComparisonPlan:
    prior = plan_fixture()
    cases = tuple(
        case.model_copy(
            update={
                "prompt": f"Revision development-only test input {i}.",
    ...
    return ComparisonPlan(
        suite=ComparisonSuite(
            cases=cases, prior_prompt_digests=tuple(digest(f"prior-{i}") for i in range(18))
        ),
...
def test_revision_pins_current_baseline_and_redacts_private_inputs() -> None:
    plan = revision_fixture()
    assert plan.baseline == "novel-opaque-output-v1"
    assert plan.candidate == "context-opaque-output-v2"
...
def test_each_prompt_is_consumed_even_after_reordering_or_partial_replacement(
    ...
    with pytest.raises(ValueError, match="already consumed"):
        consume_corpus(tmp_path, revision_fixture())
...
def test_failed_fresh_process_result_is_never_complete_or_true_negative(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "pajin.benchmark.effectiveness_revision",
            "report",
            "--root",
            str(tmp_path),
            "--reference",
