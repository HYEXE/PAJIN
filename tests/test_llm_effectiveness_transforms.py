"""EFFECT-006 trust and compatibility tests; no simulated model-quality claim."""
...
from pajin.benchmark.effectiveness_transforms.evidence import (
    ComparisonIndex,
    _quality_improved,
    recompute_comparison,
)
from pajin.benchmark.effectiveness_transforms.plan import (
    ComparisonPlan,
    ComparisonSuite,
    comparison_pins,
)
from pajin.benchmark.effectiveness_transforms.runner import consume_corpus
...
def transform_fixture() -> ComparisonPlan:
    prior = plan_fixture()
    cases = tuple(
        case.model_copy(
            update={
                "prompt": f"Transform development-only test input {i}.",
    ...
    return ComparisonPlan(
        suite=ComparisonSuite(
            cases=cases, prior_prompt_digests=tuple(digest(f"prior-{i}") for i in range(72))
        ),
...
def test_derived_pins_current_baseline_and_redacts_private_inputs() -> None:
    plan = transform_fixture()
    assert plan.baseline == "public-derived-opaque-output-v4"
    assert plan.candidate == "public-transform-opaque-output-v5"
...
def test_each_prompt_is_consumed_even_after_reordering_or_partial_replacement(
    ...
    with pytest.raises(ValueError, match="already consumed"):
        consume_corpus(tmp_path, transform_fixture())
...
def test_failed_fresh_process_result_is_never_complete_or_true_negative(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "pajin.benchmark.effectiveness_transforms",
            "report",
            "--root",
            str(tmp_path),
            "--reference",
