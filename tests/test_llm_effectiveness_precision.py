"""EFFECT-004 trust and compatibility tests; no simulated model-quality claim."""
...
from pajin.benchmark.effectiveness_precision.evidence import ComparisonIndex, recompute_comparison
from pajin.benchmark.effectiveness_precision.plan import (
    ComparisonPlan,
    ComparisonSuite,
    comparison_pins,
)
from pajin.benchmark.effectiveness_precision.runner import consume_corpus
from pajin.benchmark.effectiveness_precision.scoring import DetectorInput, measure_pair, verify_pair
from pajin.runtime.store import RunStore


def precision_fixture() -> ComparisonPlan:
    prior = plan_fixture()
    cases = tuple(
        case.model_copy(
            update={
                "prompt": f"Precision development-only test input {i}.",
    ...
    return ComparisonPlan(
        suite=ComparisonSuite(
            cases=cases, prior_prompt_digests=tuple(digest(f"prior-{i}") for i in range(36))
        ),
...
def test_precision_pins_current_baseline_and_redacts_private_inputs() -> None:
    plan = precision_fixture()
    assert plan.baseline == "novel-opaque-output-v1"
    assert plan.candidate == "mixed-group-opaque-output-v3"
...
def test_current_baseline_is_not_the_old_marker() -> None:
    ...
    assert not pair.baseline and pair.candidate
    verify_pair(value, pair)
    with pytest.raises(ValueError):
        verify_pair(value, pair.model_copy(update={"baseline": True}))
...
def test_each_prompt_is_consumed_even_after_reordering_or_partial_replacement(
    ...
    with pytest.raises(ValueError, match="already consumed"):
        consume_corpus(tmp_path, precision_fixture())
...
def test_failed_fresh_process_result_is_never_complete_or_true_negative(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "pajin.benchmark.effectiveness_precision",
            "report",
            "--root",
            str(tmp_path),
            "--reference",
