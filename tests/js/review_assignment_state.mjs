...
globalThis.crypto.randomUUID = () => fixture.acked.requestKey.slice("review-".length);
...
assert.equal(requests[0].url, `/v2/measured-reviews/${fixture.opened.reviewId}/notification-ack`);
assert.equal(JSON.parse(requests[0].options.body).assignmentRevision, 2);
const ackRequest = requests.shift();
const ackBody = JSON.parse(ackRequest.options.body);
assert.equal(ackBody.expectedRevision, undefined, "immutable assignment acknowledgment does not consume a review revision");
ackRequest.reject(new Error("receipt response lost")); await flush(); await marking;
const retryAck = mark.fire("click");
assert.deepEqual(JSON.parse(requests[0].options.body), ackBody, "receipt retry retains the exact key and assignment");
await respond(fixture.acked);
assert.equal(requests[0].url, "/v2/measured-review-inbox");
await respond(fixture.read); await retryAck;
...
assert.equal(element("inbox").querySelectorAll("button").length, 2);
assert.equal(element("inbox").querySelectorAll("button").at(-1).textContent, "Mark read");
...
assert.equal(announcements.filter(([, state]) => state === "error").length, 2);
