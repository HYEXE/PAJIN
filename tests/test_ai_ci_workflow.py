...
_WORKFLOW = Path(__file__).parents[1] / ".github" / "workflows" / "ai-002d-conformance.yml"
...
def test_ai_002d_conformance_is_manual_exact_commit_and_opt_in() -> None:
    workflow = _workflow()
    ...
    conformance_input = inputs["confirm_ai_002d_conformance"]
    assert isinstance(conformance_input, dict)
    assert conformance_input == {
        "description": "Run the exact AI-002D real-Docker conformance",
        "required": "true",
        "default": "false",
        "type": "boolean",
    }
    ...
    assert workflow["concurrency"] == {
        "group": "ai-002d-conformance-${{ github.ref }}",
...
def test_ai_002d_conformance_builds_runs_and_audits_exact_boundary() -> None:
    workflow = _workflow()
    jobs = workflow["jobs"]
    assert isinstance(jobs, dict)
    job = jobs["conformance"]
    ...
    assert confirmation["env"] == {
        "CONFIRM_AI_002D_CONFORMANCE": ("${{ inputs.confirm_ai_002d_conformance }}")
    }
    assert confirmation["run"] == 'test "$CONFIRM_AI_002D_CONFORMANCE" = "true"'
    ...
    for image, context in (
        ("pajin-ai-target:dev", "containers/ai-target"),
    ...
    conformance = named_steps["Run AI-002D conformance"]
    assert conformance["env"] == {"PAJIN_AI_002D_REAL_DOCKER": "1"}
    assert (
        "tests/test_ai_measured_product_docker.py::"
        "test_real_docker_ai_002d_exact_commit_product_conformance" in conformance["run"]
    )

    residue = named_steps["Require zero PAJIN AI Docker residue"]
    ...
    for query in (
        "label=pajin.ai-fixture.managed=true",
        "label=pajin.execution-id",
        "name=^/pajin-ai-target-",
        "name=^pajin-ai-net-",
        "name=^/pajin-proxy-",
        "name=^pajin-egress-",
    ):
