"""EFFECT-007 trust and compatibility tests; no simulated model-quality claim."""
...
from pajin.benchmark.effectiveness_structured.evidence import (
    ComparisonIndex,
    _quality_improved,
    recompute_comparison,
)
from pajin.benchmark.effectiveness_structured.plan import (
    ComparisonPlan,
    ComparisonSuite,
    comparison_pins,
)
from pajin.benchmark.effectiveness_structured.runner import consume_corpus
...
def structured_fixture() -> ComparisonPlan:
    prior = plan_fixture()
    cases = tuple(
        case.model_copy(
            update={
                "prompt": f"Structured development-only test input {i}.",
    ...
    return ComparisonPlan(
        suite=ComparisonSuite(
            cases=cases, prior_prompt_digests=tuple(digest(f"prior-{i}") for i in range(90))
        ),
...
def test_derived_pins_current_baseline_and_redacts_private_inputs() -> None:
    ...
    assert plan.baseline == "public-transform-opaque-output-v5"
    assert plan.candidate == "public-structured-opaque-output-v6"
...
def test_each_prompt_is_consumed_even_after_reordering_or_partial_replacement(
    ...
    with pytest.raises(ValueError, match="already consumed"):
        consume_corpus(tmp_path, structured_fixture())
...
def test_failed_fresh_process_result_is_never_complete_or_true_negative(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "pajin.benchmark.effectiveness_structured",
            "report",
            "--root",
            str(tmp_path),
            "--reference",
