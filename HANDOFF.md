...
2026-09-09 새 goal의 5개 과제는 `PLAN.md`에 명시한 범위의 구현·검증을 완료했다. 이전 8개 개선을 반복하지 않는다.
...
기존 제품의 전체 로컬 회귀와 추가 Linux 변경의 관련 회귀·실제 DB/Docker 검증을 완료했다.
추가 Linux 검증 변경을 포함한 `215d4fc`의 일반 CI·Web/Network/AI 원격 검증도 완료했다.
...
승인 범위의 commit·push·동일 커밋 검증을 모두 마쳤다. 구성 선택이나 실행 승인은 대기하지 않는다.
운영 배포·운영 데이터 변경 승인은 아니다. 다음 작업은 이 완료 범위 밖의 독립 과제를 선정하는 것이다.
...
- 이번 58개 변경의 여섯 commit·`origin/main` push·일반 CI와 Web/Network/AI Docker workflow 실행을
  ...
  이후 결과 문서를 보존하면서 Linux 검증 실행기·테스트 전용 이미지/lock·ADR·계약을 추가했다.
  추가 14개 파일은 별도 승인받은 한 commit `215d4fc03e1385c64ccc1e4e7fe70efd0ea4add2`로
  push했다. local HEAD·upstream·실제 원격 main이 일치했고 push 직후 worktree는 깨끗했다.
  최종 원격 결과를 반영한 이 문서·`PLAN.md`·`KNOWN_ISSUES.md`·OPS-002의 Markdown 4개만
  로컬 미커밋 변경이다. staging·삭제·untracked·진행 중인 Git 작업은 없다. 제품 `src`·기존 lock은
  추가 Linux 검증 변경에서 바뀌지 않았다. 추가 commit·push나 운영 배포는 수행하지 않았다.
...
- 최종 `215d4fc03e1385c64ccc1e4e7fe70efd0ea4add2`의 원격 검증을 완료했다. `72bdbd9` push 뒤
  fixed로 바뀐 기존 Dependabot 경고 2~7번은 새 push 뒤에도 모두 fixed였고 열린 경고는 0건이다.
  로컬 취약 동작 회귀·패키징과 실제 원격 경고 해소는 각각의 근거를 보존한다.
- [CI 34466459329](https://github.com/HYEXE/PAJIN/actions/runs/34466459329): 첫 시도에 Quality와 24 shard 모두 성공.
  **8,260 passed·기존 76 skipped**, 전체 job 실행 구간 490초다.
  로그와 duration artifact 24개에서 동일 SHA·clean tree·exit 0, 8,336개 테스트의 중복 없는 처리를
  확인했다. 기존 8,314개 항목은 모두 보존됐고 Linux cold archive/실행기 경계 테스트 22개가 추가됐다.
- 동일 커밋의 실제 Ubuntu 24.04 / Linux amd64 / Python 3.12.14 Docker 검증도 모두 첫 시도에 통과했다.

  | Workflow | 실제 검사 | 결과 |
  | --- | --- | --- |
  | [Web 34466552572](https://github.com/HYEXE/PAJIN/actions/runs/34466552572) | source·controlled validation·거부·product/fresh process | 1 passed / 136.13초 |
  | [Network 34466556691](https://github.com/HYEXE/PAJIN/actions/runs/34466556691) | 6 source·6 Replay·floor·product/fresh process | 1 passed / 270.42초 |
  | [AI 34466560216](https://github.com/HYEXE/PAJIN/actions/runs/34466560216) | source·Replay 2개·Controls 3개·floor·product/fresh process | 1 passed / 91.39초 |

  각 workflow의 명시적 확인·clean commit·이미지 ID 기록·실제 검사·무조건 실행되는 zero-residue
  단계가 모두 성공했다. Web의 ZAP digest pin도 유지했다. Worker 보고와 별도 자원 검사를 구분한다.
- private 근거는 `.pajin/five-improvements-remote/215d4fc03e1385c64ccc1e4e7fe70efd0ea4add2/`에 있다.
  `ci-verified.json`, 세 `*-verified.json`, 원본 run/jobs/로그, 24개 duration artifact,
  `dispatch.json`, `dependabot-final.json`, `linux-live-source-binding.json`을 보존했다.
  원문·모델·canary·private deployment inventory는 커밋이나 공개 artifact에 포함하지 않았다.
- 이 커밋의 `conformance-requirements.json`은 Web/Network/AI 모두 요구한다. 도구의
  `dispatchAuthorized: false`는 자동 실행 권한을 부여하지 않는다는 뜻이며 실제 사용자 승인과 구분한다.
  최종 결과 Markdown 4개만 바뀐 상태는 추가 Docker 실행 대상이 아니며 문서 검사를 별도로 적용한다.
- 이전 `72bdbd9`와 `e7c8243`/`b359c3c`의 검증은 해당 커밋의 기록이다. 새 커밋 결과로 재사용하지 않았다.
...
- 로컬 source inventory **1,513개 파일**이 실행 전후 일치했다. 이 중 저장소 파일 840개는 게시한
  `215d4fc`의 bytes와 일치한다. 나머지 673개는 이미지에서 사용하지 않는 ignored vendor이며,
  Worker 의존성은 커밋된 hash lock으로 이미지 안에서 설치한다. 전체 inventory SHA-256은
...
- 추가 변경의 정책은 Web/Network/AI 모두 요구했으며 `215d4fc`의 실제 원격 검증으로 충족했다.
  `72bdbd9`의 이전 성공을 재사용하지 않았다. 운영 배포는 범위 밖이다.
...
- Linux 검증 추가 전 `72bdbd9` 소스의 로컬 `.venv/bin/python -m pytest -q`:
  **8,238 passed, 76 skipped / 2,270.35초**, exit 0. 추가분은 위 125개 관련 회귀와 최종 원격 전체 CI로 검증했다.
...
- 이번 58개 파일은 모두 이 goal의 수정·추가이며 삭제나 진행 중인 merge/rebase 등은 없다.
  ...
  앞선 여섯 commit에 이어 Linux 검증 추가분 14개 파일의 한 commit·push·원격 검증도 완료했다.
  최종 원격 결과 문서 4개만 로컬 변경이며, 마지막 문서 검사·whitespace 검사를 별도로 수행했다.
...
최초 58개 변경 파일은 여섯 단위로, Linux 검증 추가 14개 파일은 별도 한 커밋으로 저장했다.
정확한 파일 목록은 private `.pajin/five-improvements-commit-plan.json`과
`.pajin/ops002-linux-approved-commit.json`에 있다. 해당 commit·push·workflow 실행을 각각 승인받았다.
...
7. `215d4fc` — `test(operations): Linux 혼합 구성의 복원과 중단을 실제 검증`; 별도 승인받은 추가 14개 파일.

승인 대상 `origin/main` 반영과 동일 커밋의 일반 CI·세 수동 workflow 확인을 완료했다.
각 `confirm_*_002d_conformance=true` 입력과 실제 run ID는 해당 커밋의 private `dispatch.json`에 보존했다.
이는 GitHub main 갱신과 CI/Docker runner 실행 승인이다. DB·호스트 구성은 별도로 선정했으며 운영 배포는 승인·실행하지 않았다.
