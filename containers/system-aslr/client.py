...
def execute() -> dict[str, object]:
    if sys.argv[1:] != ["system-aslr"] or os.getuid() != 65532:
    ...
    if (
        ...
        or target.query
        or target.fragment
        or target.path != "/v1/aslr"
        or value["operation"] != "linux-aslr-v1"
...
def read_agent(
    ...
    client_pin = sha256(ssl.PEM_cert_to_DER_cert(credentials["certificate"])).hexdigest()
    with TemporaryDirectory(prefix="sys-004-", dir="/tmp") as directory:
        ...
        try:
            ...
            return {
                "schema": "pajin.sys-004.worker-read/v1",
                "agent": result,
