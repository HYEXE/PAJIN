...
# Resolve one marker-preserving lock for every supported platform and CPU. The
# Worker image is Linux-only, so pip ignores the non-Linux branches at install
# time while retaining a reproducible graph for both amd64 and arm64 builds.
...
Write-Host "Updated hash-locked Worker dependencies in $LockFile"
