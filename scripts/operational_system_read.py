"""Explicit owned SYS-002 Docker fixture; no caller-supplied target or cloud credentials."""
...
def inventory() -> dict[str, str]:
    paths = [
        p
        for directory in ("src/pajin", "containers/system-agent")
        for p in Path(directory).rglob("*")
        if p.is_file() and (p.suffix == ".py" or p.name == "Dockerfile")
    ]
    paths += [
        Path("scripts/operational_system_read.py"),
        Path("tests/operational_system_read_probe.py"),
        Path("tests/sys_002_support.py"),
    ]
...
def main() -> int:
    ...
    owner = "pajin.sys002-owner=" + uuid4().hex
    before = inventory()
        ...
        with (output / "pytest.log").open("x") as log:
            code = subprocess.run(
                [
                    ...
                    "-q",
                    "--tb=short",
                    "tests/operational_system_read_probe.py",
                ],
                env={
                    **os.environ,
                    "PAJIN_SYS002_ROOT": str(output),
                    "PAJIN_SYS002_OWNER": owner,
                    "PAJIN_SYS002_IMAGE": args.image_id,
                    "PAJIN_SYS002_PROXY": args.proxy_image_id,
                },
                stdout=log,
    ...
    print(
        json.dumps(
            {"exitCode": code, "sourceUnchanged": before == after, "cleanup": cleanup},
            sort_keys=True,
        )
    )
    return code if before == after else 1
